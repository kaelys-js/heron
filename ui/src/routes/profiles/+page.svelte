    ...
    <div class="max-w-3xl mx-auto space-y-5">
      ...
      <div class="space-y-1.5 max-w-2xl">
        ...
        <p class="text-sm text-muted-foreground leading-relaxed">
          Each profile is a distinct career track with its own CV, target roles, filters, pipeline,
          and applications tracker. Shared infrastructure (API keys, LinkedIn / Indeed sessions,
          Gmail IMAP, autopilot schedule) is reused across every profile.
        </p>
      </div>
              ...
              <div class="flex items-start gap-3">
                  ...
                  <div class="flex items-center gap-3 text-[11px] text-muted-foreground mt-1">
                    ...
                    {#if p.lastActiveAt}
                      <span class="text-muted-foreground/60"
                        >· last active {formatRelativeTime(p.lastActiveAt)} ago</span
                      >
                    {/if}
                  </div>
                ...
                <div class="flex flex-col gap-1.5 flex-shrink-0">
                  {#if !isActive}
                    ...
                    >
                      {#if b === 'activate'}<Loader2
                          class="size-3 animate-spin"
                        />{:else}<ArrowRight class="size-3" /> Activate{/if}
                    </Button>
                  {/if}
                  <Button
                    variant="ghost"
              ...
              {#if confirmDeleteId === p.id}
                <div class="mt-3 p-3 rounded-md border border-red-500/30 bg-red-500/5 space-y-2">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    >
                      {#if b === 'delete'}<Loader2 class="size-3 animate-spin" />{:else}<Trash2
                          class="size-3"
                        /> Delete{/if}
                    </Button>
                    <Button
