      ...
      <div class="flex items-start justify-between gap-4 flex-wrap">
        <div class="space-y-1.5 max-w-2xl">
          ...
          <p class="text-sm text-muted-foreground leading-relaxed">
            Each project bundles a filter — score, background-check risk, search text, "has report"
            flags — into a named view. Open a project to see the matching slice of your pipeline;
            track an application target per project to keep parallel job-hunting tracks moving
            forward.
          </p>
        </div>
      ...
      {#if data.projects.length === 0}
        ...
        <Card.Root class="border-dashed">
          <Card.Content class="p-8 flex flex-col items-center text-center gap-3">
            ...
            <p class="text-xs text-muted-foreground max-w-md leading-relaxed">
              Try one of these starter templates — each one is a useful slice of {data.totalJobs.toLocaleString()}
              jobs in your pipeline. You can edit, rename, or delete them later.
            </p>
          </Card.Content>
        </Card.Root>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          {#each data.starters as starter}
            <Card.Root
              class={cn(
                'border-l-2 transition-colors hover:bg-accent/30',
                COLOR_BORDER_L[starter.color],
              )}
            >
              <Card.Header class="pb-3">
                <div class="flex items-start gap-3">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <Card.Description class="text-xs mt-1 leading-relaxed"
                      >{starter.description}</Card.Description
                    >
                  </div>
                </div>
              </Card.Header>
              <Card.Content class="pt-0 pb-4">
                <Button
                  variant="outline"
                  size="sm"
                  class="w-full h-8 gap-1.5"
                  onclick={() => openCreate(starter as Partial<Project>)}
                >
      ...
      {:else}
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {#each data.projects as p (p.id)}
            ...
            {@const progress =
              p.target > 0 ? Math.min(100, (stats.applied / p.target) * 100) : null}
            <Card.Root
              class={cn(
                'border-l-2 overflow-hidden flex flex-col transition-colors hover:border-accent/60',
                COLOR_BORDER_L[p.color],
              )}
            >
              <Card.Header class="pb-3">
                <div class="flex items-start gap-2">
                  <div
                    class={cn(
                      'size-7 rounded-md flex items-center justify-center ring-1 flex-shrink-0 text-[11px] font-mono uppercase font-semibold',
                      COLOR_TINT[p.color],
                    )}
                  >
                    {p.name.slice(0, 2)}
                  ...
                  <div class="space-y-1">
                    <div class="flex items-center justify-between text-[11px]">
                      ...
                      <span class="text-muted-foreground tabular-nums"
                        >{stats.applied}/{p.target} applied</span
                      >
                    </div>
  ...
  <Sheet.Content side="right" class="w-full sm:max-w-lg flex flex-col p-0 gap-0">
    <Sheet.Header class="px-6 pt-6 pb-4 border-b">
      <div class="flex items-start gap-3">
        <div
          class={cn(
            'size-9 rounded-lg ring-1 flex items-center justify-center flex-shrink-0',
            COLOR_TINT[editor.color],
          )}
        >
            ...
            <div class="flex items-center gap-1.5 flex-wrap">
              {#each COLORS as c}
                <Tooltip.Root>
                  ...
                  <Tooltip.Content side="top" class="text-xs capitalize"
                    >{c}{editor.color === c ? ' · selected' : ''}</Tooltip.Content
                  >
                </Tooltip.Root>
              {/each}
            </div>
      ...
      <div class="space-y-3 pt-2 border-t">
        ...
        <div class="flex items-center justify-between pt-1">
          ...
          <Switch
            ...
            onCheckedChange={(v: boolean) =>
              (editor = { ...editor, filter: { ...editor.filter, hasReport: !!v } })}
          />
        </div>
        ...
        <div class="flex items-center justify-between">
          ...
          <Switch
            ...
            onCheckedChange={(v: boolean) =>
              (editor = { ...editor, filter: { ...editor.filter, hasPdf: !!v } })}
          />
        </div>
      </div>
      ...
      <div class="space-y-3 pt-2 border-t">
        ...
        <div class="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-3 items-start">
          <p class="text-xs text-muted-foreground leading-relaxed">
            How many applications do you want to send from this project? Set 0 to skip the target
            tracker.
          </p>
          <Stepper
            value={editor.target}
    ...
    <Sheet.Footer class="px-6 py-3 border-t">
      <Button variant="ghost" onclick={() => (editorOpen = false)} disabled={editorBusy}
        >Cancel</Button
      >
