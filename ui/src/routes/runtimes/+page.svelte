    ...
    <div class="max-w-5xl mx-auto space-y-5">
      ...
      <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div class="space-y-1.5 max-w-2xl">
          ...
          <p class="text-sm text-muted-foreground leading-relaxed">
            Every runtime and integration the pipeline depends on, with live status. The pipeline can run on Node + Python + Gemini alone — Claude and Adzuna are optional accelerators.
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
        <div class="rounded-lg border border-emerald-500/30 bg-emerald-500/5 px-4 py-3">
          <div class="flex items-center gap-2">
            ...
            <span class="text-[11px] uppercase tracking-wide text-emerald-300/80 font-medium">Healthy</span>
          </div>
        ...
        <div class={cn(
          ...
          <div class="flex items-center gap-2">
            <AlertTriangle class={cn('size-4', stats.issues > 0 ? 'text-red-400' : 'text-muted-foreground/50')} />
            <span class={cn(
              'text-[11px] uppercase tracking-wide font-medium',
              stats.issues > 0 ? 'text-red-300/80' : 'text-muted-foreground/70'
            )}>Issues</span>
          </div>
          <div class={cn('text-2xl font-mono tabular-nums mt-1', stats.issues > 0 ? 'text-red-200' : 'text-muted-foreground')}>{stats.issues}</div>
        ...
        <div class="rounded-lg border border-zinc-500/30 bg-zinc-500/5 px-4 py-3">
          <div class="flex items-center gap-2">
            ...
            <span class="text-[11px] uppercase tracking-wide text-zinc-300/80 font-medium">Not set up</span>
          </div>
        ...
        <div class="rounded-lg border border-blue-500/30 bg-blue-500/5 px-4 py-3">
          <div class="flex items-center gap-2">
            <Activity class={cn('size-4 text-blue-400', data.report.summary.runningTasks.length > 0 && 'animate-pulse')} />
            <span class="text-[11px] uppercase tracking-wide text-blue-300/80 font-medium">Running</span>
          </div>
          <div class="text-2xl font-mono tabular-nums mt-1 text-blue-200">{data.report.summary.runningTasks.length}</div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        {#each data.report.cards as c}
          ...
          <Card.Root class={cn('overflow-hidden border-l-2 transition-colors', STATUS_BORDER[c.status])}>
            <Card.Header class="pb-3">
              <div class="flex items-start gap-3">
                <div class={cn('size-9 rounded-lg flex items-center justify-center ring-1 flex-shrink-0', STATUS_RING[c.status])}>
                  <Icon class="size-4" />
                </div>
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2 flex-wrap">
                    ...
                    {#if c.required}
                      <span class="text-[10px] font-medium text-red-400/90 uppercase tracking-wide">required</span>
                    {:else}
                      <span class="text-[10px] font-medium text-muted-foreground/60 uppercase tracking-wide">optional</span>
                    {/if}
                  </div>
                  <div class="flex items-center gap-2 mt-1">
                    ...
                    {#if c.badge}
                      ...
                      <span class="text-[11px] font-mono text-muted-foreground/80 truncate">{c.badge}</span>
                    {/if}
                  </div>
                </div>
              </div>
              ...
              {#if c.usage}
                <div class="flex items-center justify-between gap-2 pt-2 border-t border-border/40">
                  ...
                  <div class="text-[10px] text-muted-foreground">
                    {c.usage.lastUsedAt ? 'last ' + formatRelativeTime(c.usage.lastUsedAt) : 'no recent calls'}
                  </div>
                </div>
                {#if c.usage.lastError}
                  <div class="flex items-start gap-1.5 text-[11px] px-2 py-1.5 rounded border border-red-500/30 bg-red-500/5 text-red-300">
                    <AlertTriangle class="size-3 mt-0.5 flex-shrink-0" />
                    <div class="flex-1 min-w-0">
                      <div class="font-medium overflow-hidden whitespace-nowrap">{c.usage.lastError.title}</div>
                      {#if c.usage.lastError.message}
                        <div class="text-red-300/80 overflow-hidden whitespace-nowrap">{c.usage.lastError.message}</div>
                      {/if}
                      <div class="text-red-400/60 mt-0.5">{formatRelativeTime(c.usage.lastError.ts)}</div>
                    </div>
                  </div>
                {/if}
              {/if}
              ...
              <div class="flex items-center gap-2 pt-1">
                ...
                {#if c.status === 'down' || c.status === 'degraded'}
                  <span class="text-[11px] text-amber-400 flex items-center gap-1">
                    ...
                    {c.status === 'down' ? 'Action required to restore' : 'Some features unavailable'}
                  </span>
                {/if}
              </div>
            </Card.Content>
