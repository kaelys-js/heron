    ...
    <div class="max-w-5xl mx-auto space-y-5">
      ...
      <div class="space-y-1.5">
        ...
        <p class="text-sm text-muted-foreground leading-relaxed max-w-3xl">
          Live view of the autonomous-apply pipeline. Jobs land here when you click Apply
          (or auto-queue from a Ready CV). The apply-queue drain runs daily on weekdays;
          you can also kick it off manually with the button below. Soft-failures (CAPTCHA,
          anti-bot, unknown fields) flow to the Inbox with a "Open posting" CTA so you can
          finish them by hand.
        </p>
      </div>

      <!-- Counts header -->
      <div class="rounded-md border border-border/40 bg-card px-4 py-3 flex items-center gap-4 flex-wrap">
        ...
        <div class="flex items-center gap-1.5 text-xs">
          <Loader2 class={cn('size-3.5 text-blue-400', data.applying.length > 0 && 'animate-spin')} />
      ...
      {#if data.applying.length > 0}
        <section class="space-y-2">
          ...
          {#each data.applying as job (job.id)}
            ...
            <div class="flex items-start gap-3 rounded-md border border-blue-500/30 bg-blue-500/5 p-3">
      ...
      {#if data.queued.length > 0}
        <section class="space-y-2">
          ...
          <div class="rounded-md border border-border/40 bg-muted/10 px-3 py-2 flex items-center gap-2 flex-wrap text-[11px] text-muted-foreground">
            <input
              type="checkbox"
            ...
            />
            <span>{selected.size} / {data.queued.length} selected (for legacy batch-send below)</span>
        ...
        <section class="space-y-2">
          ...
          <div class="rounded-md border border-amber-500/30 bg-amber-500/5 px-3 py-2 flex items-start gap-2">
            ...
            <p class="text-[11px] text-amber-100/90 leading-relaxed">
              These hit a soft block during auto-apply (CAPTCHA, anti-bot, unknown form field).
              The Inbox has a "Open posting" CTA on each one — finish them in the browser, then
              mark Applied.
            </p>
          </div>
      ...
      {#if data.queued.length > 0 && selected.size > 0}
        <section class="space-y-2">
          ...
          <p class="text-[10px] text-muted-foreground/70 leading-relaxed pl-1">
            Use this when you want LinkedIn auto-apply + new-tab open for non-LinkedIn at the
            same time. Most users prefer "Run drain now" above — that respects your per-profile
