...
<div class="h-full overflow-y-auto">
  ...
  <div class="p-6">
    <div class="max-w-4xl mx-auto space-y-5">
      <!-- Hero / master switch -->
      <Card.Root class={cn('border-l-2 transition-colors', config.globalEnabled ? 'border-l-emerald-500/60' : 'border-l-zinc-700')}>
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-muted-foreground mt-1.5 leading-relaxed">
              {#if config.globalEnabled}
                {enabledCount} {enabledCount === 1 ? 'schedule is' : 'schedules are'} active.
                {#if scheduledNext}
                  Next:
                  <span class="text-foreground">{config.schedules.find((s) => s.id === scheduledNext!.id)?.name}</span>
                  · <span class="text-emerald-300">{fmtCountdown(scheduledNext.ts)}</span>
                {:else}
                  No daily schedules enabled — only event triggers will fire.
                {/if}
              {:else}
                Autopilot is paused. Even with individual schedules enabled, nothing fires until this is on.
              {/if}
            </p>
          </div>
        </Card.Content>
      ...
      {#if config.globalEnabled}
        <div class="flex items-start gap-2 px-3 py-2 rounded-md border border-blue-500/30 bg-blue-500/5 text-xs">
          <Info class="size-3.5 text-blue-300 mt-0.5 flex-shrink-0" />
          <div class="text-blue-200/80">
            The scheduler runs in-process while this dashboard is open. For 24/7 background runs, set up a macOS launchd agent that opens the dashboard at boot — or run scheduled tasks via cron instead.
          </div>
        </div>
      {/if}
      ...
      <!-- Schedule cards -->
      <div class="space-y-3">
        <h2 class="text-[11px] font-medium tracking-wider text-muted-foreground/80 uppercase">Schedules</h2>
            ...
            <Card.Content class="p-4 space-y-3">
              <div class="flex items-start gap-3">
                <div class={cn(
                  ...
                  s.enabled ? 'ring-border/40 bg-muted/40' : 'ring-border/20 bg-muted/20 opacity-60'
                )}>
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2 flex-wrap">
                    ...
                    {#if isAfter}
                      <span class="text-[10px] uppercase tracking-wide text-muted-foreground/60">event-triggered</span>
                    {/if}
                    {#if s.lastRunResult === 'started'}
                      <span class="inline-flex items-center gap-1 text-[10px] uppercase tracking-wide text-blue-300">
                        <Activity class="size-2.5 animate-pulse" /> running
                      </span>
                    {/if}
                  </div>
                  <p class="text-xs text-muted-foreground mt-0.5 leading-relaxed">{s.description}</p>
                  <button
                    type="button"
                  ...
                  >
                    <ChevronDown class={cn('size-3 transition-transform', isExpanded && 'rotate-180')} />
                  ...
                  {#if !isAfter}
                    <Tooltip.Provider delayDuration={200}>
                      <Tooltip.Root>
                        ...
                        <Tooltip.Content side="bottom" class="text-xs">Trigger this schedule immediately</Tooltip.Content>
                      </Tooltip.Root>
                    </Tooltip.Provider>
                  {/if}
                  <Switch
                ...
                <div class="overflow-hidden min-h-0">
                  <ul class="pl-12 pt-1 pb-1 space-y-1">
                    {#each s.details as line}
                      <li class="flex items-start gap-2 text-[11px] text-muted-foreground leading-relaxed">
              ...
              {#if !isAfter && s.trigger.type === 'daily'}
                ...
                <div class="pl-12 pt-3 border-t border-border/40 space-y-3">
                  <div class="flex items-center gap-3 flex-wrap">
                    <Label class="text-[10px] uppercase tracking-wider text-muted-foreground/80 w-20">Time</Label>
                    <TimePicker
                      hour={s.trigger.hour}
                  ...
                  <div class="flex items-start gap-3 flex-wrap">
                    <Label class="text-[10px] uppercase tracking-wider text-muted-foreground/80 w-20 mt-1.5">Days</Label>
                      ...
                      <Tooltip.Provider delayDuration={250}>
                        <div class="flex items-center gap-1">
                          {#each WEEKDAY_LABELS as label, day}
                            {@const isOn = s.trigger.type === 'daily' && (s.trigger.weekdays.length === 0 || s.trigger.weekdays.includes(day))}
                            {@const allDays = s.trigger.type === 'daily' && s.trigger.weekdays.length === 0}
                            <Tooltip.Root>
                              <Tooltip.Trigger>
                                {#snippet child({ props })}
                                  <button
                              ...
                              <Tooltip.Content side="top" class="text-xs">
                                {WEEKDAY_FULL[day]}{allDays ? ' (every day)' : isOn ? ' · click to disable' : ' · click to enable'}
                              </Tooltip.Content>
                            </Tooltip.Root>
                          {/each}
                        </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-3 items-start">
            <div class="space-y-1">
              ...
              <p class="text-[11px] text-muted-foreground leading-relaxed">
                ...
                <span class="text-muted-foreground/70">Range 0.0–5.0. <span class="text-emerald-400/80">4.0 is the standard cutoff</span> — lower casts a wider net at higher cost.</span>
              </p>
            </div>
            <Stepper
              value={config.thresholds.autoEvaluateScore}
              onchange={(v) => (config = { ...config, thresholds: { ...config.thresholds, autoEvaluateScore: v } })}
              min={0}
              max={5}
              step={0.1}
              decimals={1}
          ...
          <div class="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-3 items-start">
            <div class="space-y-1">
              ...
              <p class="text-[11px] text-muted-foreground leading-relaxed">
                ...
                <span class="text-muted-foreground/70">Above ~50/day risks LinkedIn flagging the account — keep this conservative.</span>
              </p>
            </div>
            <Stepper
              value={config.thresholds.maxAppliesPerDay}
              onchange={(v) => (config = { ...config, thresholds: { ...config.thresholds, maxAppliesPerDay: v } })}
              min={1}
              max={100}
              step={1}
              decimals={0}
      ...
      <!-- Save bar -->
      {#if dirty}
        <div class="sticky bottom-4 flex items-center gap-2 px-4 py-2.5 rounded-lg border border-amber-500/40 bg-amber-500/10 backdrop-blur-md shadow-lg">
      ...
      <Card.Root>
        <Card.Header class="pb-3">
          ...
          <Card.Description class="text-xs">Last 50 events from Autopilot, scan, gemini, and apply tasks.</Card.Description>
        </Card.Header>
        <Card.Content>
                ...
                <li class="flex items-start gap-2.5 text-xs">
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-baseline gap-2">
                      ...
                      <span class="text-[10px] text-muted-foreground ml-auto flex-shrink-0">{formatRelativeTime(ev.ts)}</span>
                    </div>
                    {#if ev.message}
                      <p class="text-[11px] text-muted-foreground mt-0.5 leading-relaxed overflow-hidden whitespace-nowrap">{ev.message}</p>
                    {/if}
