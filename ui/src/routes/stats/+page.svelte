  ...
  <div class="p-6">
    <div class="max-w-6xl mx-auto space-y-6">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Card.Root>
          ...
          <Card.Content class="space-y-1">
            ...
            <div class="text-[11px] text-muted-foreground">
              {data.reports} reports · {data.pdfs} PDFs
            </div>
          </Card.Content>
        </Card.Root>
        ...
        <Card.Root>
          <Card.Header class="pb-2">
            <Card.Description class="text-[11px] uppercase tracking-wide"
              >Avg score</Card.Description
            >
          </Card.Header>
          <Card.Content class="space-y-1">
            <div class="text-2xl font-semibold tabular-nums">{data.avgScore.toFixed(2)}</div>
            <div class="text-[11px] text-muted-foreground">
              across {data.counts.total - data.unscored} scored
            </div>
          </Card.Content>
        </Card.Root>
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4 pt-4 border-t border-border">
            <div>
              ...
              <div class="text-sm font-semibold tabular-nums">
                {pct(data.conversion.scoredOfTotal)}
              </div>
            </div>
            <div>
              <div class="text-[10px] text-muted-foreground uppercase tracking-wide">
              ...
              <div class="text-sm font-semibold tabular-nums">
                {pct(data.conversion.readyOfScored)}
              </div>
            </div>
            <div>
              <div class="text-[10px] text-muted-foreground uppercase tracking-wide">
              ...
              <div class="text-sm font-semibold tabular-nums">
                {pct(data.conversion.appliedOfReady)}
              </div>
            </div>
            <div>
              <div class="text-[10px] text-muted-foreground uppercase tracking-wide">
              ...
              <div class="text-sm font-semibold tabular-nums">
                {pct(data.conversion.interviewOfApplied)}
              </div>
            </div>
          </div>
        </Card.Content>
        ...
        <Card.Root>
          <Card.Header class="pb-2">
            ...
            <Card.Description class="text-xs">
              Daily new-jobs found across every scan source. Last run: {data.scanHistory
                .lastRunDate ?? '—'}.
            </Card.Description>
          </Card.Header>
            ...
            <div class="flex items-end gap-1 h-20">
              {#each data.scanHistory.recent.slice().reverse() as run (run.date)}
                ...
                <div
                  ...
                  title={run.date + ' · ' + run.added + ' new'}
                >
                  <div
                    class="w-full bg-emerald-500/40 hover:bg-emerald-500/60 transition-colors rounded-sm"
                    style="height: {h}px"
                  ></div>
                </div>
              {/each}
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <Card.Root>
          <Card.Header class="pb-2">
            ...
            <Card.Description class="text-xs"
              >Stacked: applied (darker) vs unapplied. {data.unscored} unscored.</Card.Description
            >
          </Card.Header>
          <Card.Content>
            ...
            <div class="space-y-1.5">
              <Tooltip.Provider delayDuration={300}>
                {#each data.topCompanies as c}
                  ...
                  <div class="flex items-center gap-3 group/cmpny">
                    <div
                      class="w-32 text-xs overflow-hidden whitespace-nowrap shrink-0"
                      title={c.name}
                    >
                      {c.name}
                    </div>
                    ...
                    <Tooltip.Root>
                      ...
                      <Tooltip.Content side="left" class="text-xs max-w-xs">
                        Refresh open roles at {c.name} via direct Greenhouse / Ashby / Lever API. Auto-triage
                        runs after.
                      </Tooltip.Content>
                    </Tooltip.Root>
                  </div>
              ...
              {#if data.topCompanies.length === 0}
                <EmptyState
                  ...
                  variant="inline"
                  icon={Building2}
                  description="No companies yet — run a scan."
                />
              {/if}
            </div>
          </Card.Content>
            ...
            <div class="space-y-1.5">
              {#each data.topSources as s}
                ...
                <div class="flex items-center gap-3">
                  <div
                    class="w-28 text-xs overflow-hidden whitespace-nowrap shrink-0"
                    title={s.name}
                  >
                    {s.name}
                  </div>
                  <div class="flex-1 relative h-5 rounded bg-muted/30 overflow-hidden">
                    <div
                      class="absolute inset-y-0 left-0 bg-cyan-500/40"
                      style={'width: ' + pct2 + '%'}
                    ></div>
                  </div>
              ...
              {#if data.topSources.length === 0}
                <EmptyState
                  ...
                  variant="inline"
                  icon={Globe}
                  description="No source data yet — run a scan."
                />
              {/if}
            </div>
          </Card.Content>
      ...
      <Card.Root>
        <Card.Header class="pb-2 flex flex-row items-center justify-between">
          <div>
            ...
            <Card.Description class="text-xs"
              >Highest-scoring jobs awaiting application.</Card.Description
            >
          </div>
        ...
        <Card.Content>
          {#if data.topReady.length === 0}
            <EmptyState
              ...
              variant="inline"
              icon={CheckCircle2}
              description="No Ready jobs. Run Gemini scoring + a deep evaluation pass to fill the queue."
            />
          {:else}
            <div class="space-y-1.5">
              {#each data.topReady as j}
                <a
                  href={'/job/' + j.id}
                  class="flex items-center gap-3 px-2 py-1.5 rounded hover:bg-muted/40 transition-colors"
                >
                  <div class="flex-1 min-w-0">
                    <div class="text-sm font-medium overflow-hidden whitespace-nowrap">
                      {j.company}
                    </div>
                    <div class="text-xs text-muted-foreground overflow-hidden whitespace-nowrap">
                      {j.role}
                    </div>
                  </div>
                  {#if j.bgRisk}
                  ...
                  {/if}
                  {#if j.score != null}
                    <span class="text-sm font-semibold tabular-nums w-10 text-right"
                      >{j.score.toFixed(1)}</span
                    >
                  {/if}
                </a>
              {/each}
