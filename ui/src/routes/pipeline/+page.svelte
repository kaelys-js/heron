<script lang="ts">
  ...
  function applySort(jobs: Job[]): Job[] {
    ...
    switch (sort) {
      ...
      case 'score-asc':
        sorted.sort(
          (a, b) => (a.score ?? a.geminiScore ?? Infinity) - (b.score ?? b.geminiScore ?? Infinity),
        );
        break;
      case 'company-asc':
  ...
  async function runLivenessSweep() {
    ...
    try {
      ...
      toast.success('Liveness sweep queued', {
        description:
          'Walking up to ~200 URLs through Playwright. Expired postings auto-close; uncertain ones land in the Inbox. Watch the bell for completion.',
        duration: 8_000,
      });
    } catch (e) {
...
<div class="h-full overflow-y-auto">
  <Topbar title="Pipeline" bind:activeTab bind:sort bind:viewMode bind:filter />
  <div class="p-4 space-y-4">
      ...
      >
        ...
        <span class="flex-1 min-w-0">
          ...
          <span class="text-muted-foreground"
            >— some links may be dead. Run a liveness sweep to auto-close expired postings.</span
          >
        </span>
        <Button
          variant="outline"
    ...
    {:else if viewMode === 'board'}
      <div class="grid gap-3" style="grid-template-columns: repeat(auto-fill, minmax(280px, 1fr));">
        {#each visibleStatuses as status}
          <StatusColumn
            title={status}
            jobs={filtered[status] ?? []}
            tint={STATUS_TINTS[status] ?? ''}
          />
        {/each}
      </div>
    {:else if viewMode === 'list'}
      ...
      <PaginationFooter
        ...
        bind:visible={visibleCount}
        pageSize={PAGE_SIZE}
        class="rounded-md border border-border/40 bg-card mt-2"
      />
      ...
      <div class="rounded-md border border-border/40 bg-card overflow-hidden">
        ...
        <Tooltip.Provider delayDuration={300}>
          ...
          >
            ...
            <Tooltip.Root>
              ...
              <Tooltip.Content side="bottom" class="text-xs max-w-xs"
                >Where this URL was first surfaced — direct ATS scan, JobSpy aggregator, niche
                board, email alert, etc.</Tooltip.Content
              >
            </Tooltip.Root>
            <Tooltip.Root>
              <Tooltip.Trigger>
              ...
              <Tooltip.Content side="bottom" class="text-xs"
                >Posting location requirement</Tooltip.Content
              >
            </Tooltip.Root>
            <Tooltip.Root>
              <Tooltip.Trigger>
            ...
            <Tooltip.Root>
              ...
              <Tooltip.Content side="bottom" class="text-xs"
                >Background-check risk tier</Tooltip.Content
              >
            </Tooltip.Root>
            <Tooltip.Root>
              <Tooltip.Trigger>
            ...
            <Tooltip.Root>
              ...
              <Tooltip.Content side="bottom" class="text-xs"
                >Has report / Has CV PDF indicators</Tooltip.Content
              >
            </Tooltip.Root>
            <Tooltip.Root>
              <Tooltip.Trigger>
        ...
        <div class="divide-y divide-border/20">
          {#each pagedJobs as job, i (job.id)}
            <div
              class={i >= prevVisibleCount && i < visibleCount
    ...
    {:else if viewMode === 'table'}
      ...
      <PaginationFooter
        ...
        bind:visible={visibleCount}
        pageSize={PAGE_SIZE}
        class="rounded-md border border-t-0 border-border/40 bg-card -mt-px"
      />
