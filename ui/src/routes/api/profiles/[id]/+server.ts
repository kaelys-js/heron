...
import { deleteProfile, renameProfile, recolorProfile, getProfile, type ProfileColor, PROFILE_COLORS } from '$lib/server/profiles';
import { profilePath } from '$lib/server/profile-paths';
import { logEvent } from '$lib/server/events';

export const PATCH = wrap('profiles-update', async ({ params, request }: { params: { id: string }; request: Request }) => {
  const id = params.id;
...
export const DELETE = wrap('profiles-delete', async ({ params, request }: { params: { id: string }; request: Request }) => {
  const id = params.id;
  if (!getProfile(id)) badRequest('Unknown profile: ' + id);
