...
import { readConfig, patchConfig } from '$lib/server/autopilot';
import type { AutopilotConfig } from '$lib/server/autopilot';
...
export const POST = wrap('autopilot', async ({ request }: { request: Request }) => {
  ...
  if (!body || typeof body !== 'object') {
    badRequest('expected JSON body with config patch');
  }
