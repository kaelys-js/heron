...
import {
  scaffoldTakeHome, findTakeHomeForJob, updateTakeHomeState,
  type TakeHomeState,
} from '$lib/server/takehome-scaffolder';
import { resolveJobAndProfile } from '$lib/server/job-resolver';

export const GET = wrap('takehome', async ({ params, url }: { params: { id: string }; url: URL }) => {
...
export const POST = wrap('takehome', async ({ params, url, request }: { params: { id: string }; url: URL; request: Request }) => {
  const resolved = resolveJobAndProfile(params.id, url);
  if (!resolved) return { ok: false, error: 'Job not found' };
  const body = (await request.json().catch(() => ({}))) as {
    budgetMinutes?: number; briefExcerpt?: string;
  };
...
export const PATCH = wrap('takehome', async ({ params, url, request }: { params: { id: string }; url: URL; request: Request }) => {
  const resolved = resolveJobAndProfile(params.id, url);
  if (!resolved) return { ok: false, error: 'Job not found' };
  const body = (await request.json().catch(() => ({}))) as Partial<TakeHomeState> & { milestone?: string };
  ...
  if (body.milestone) {
    ...
    const milestones = [...(existing?.state.milestones ?? []), { ts: Date.now(), label: body.milestone }];
