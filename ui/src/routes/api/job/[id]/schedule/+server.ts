...
import {
  getSchedule, setSchedule, type ScheduleEntry,
} from '$lib/server/interview-schedule';
import { resolveJobAndProfile } from '$lib/server/job-resolver';

export const GET = wrap('interview-schedule', async ({ params, url }: { params: { id: string }; url: URL }) => {
...
export const POST = wrap('interview-schedule', async ({ params, url, request }: { params: { id: string }; url: URL; request: Request }) => {
  const resolved = resolveJobAndProfile(params.id, url);
  if (!resolved) return { ok: false, error: 'Job not found' };
