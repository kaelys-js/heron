...
function persist(jobId: string, body: string): string {
  fs.mkdirSync(PREP_DIR, { recursive: true });
  const filename = slugify(jobId) + '-followup.md';
  const fullPath = path.join(PREP_DIR, filename);
  fs.writeFileSync(fullPath, body);
  return path.relative(ROOT, fullPath);
}

function spawnFollowup(url: string, tone: string, jobId: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    p.on('close', (code) => {
      ...
      try {
        const persisted = persist(jobId, stdout);
        resolve(persisted);
      } catch (err) {
        reject(err);
      }
...
export const POST = wrap(
  ...
  async ({ params, request }: { params: { id: string }; request: Request }) => {
    ...
    try {
      const filePath = await spawnFollowup(job!.url, tone, job!.id);
