...
<div class="h-full overflow-y-auto">
  <Topbar title="Help · Autonomous apply" subtitle="risk acknowledgment + reference" showTabs={false} />
    ...
    <div class="max-w-3xl mx-auto space-y-6">
      ...
      <div class="space-y-2">
        <div class="flex items-center gap-3">
          <div class="size-10 rounded-lg bg-fuchsia-500/10 ring-1 ring-fuchsia-500/40 flex items-center justify-center">
        ...
        <p class="text-sm text-muted-foreground leading-relaxed">
          ...
          LinkedIn / Greenhouse / Ashby — including the final Submit click. Default: OFF. The
          system overrides the usual "stop at Submit" ethical rule only when this toggle is
          explicitly enabled on a profile, the job's score clears your minimum-score gate, the
          portal has a production adapter, and the daily apply cap hasn't been hit.
        </p>
      </div>
        ...
        <Card.Content class="space-y-3 text-sm leading-relaxed">
          <ul class="space-y-2 list-disc pl-5 text-amber-100/90">
            <li>
              ...
              the global cap exist for this reason — leave them at the defaults unless you accept the risk.
            </li>
            <li>
              <strong>Generic cover-letter quality.</strong> Each autonomous apply generates a fresh cover
              letter via Claude. Quality varies and is sometimes worse than what a human would write. The
              generated cover lives under <code class="font-mono">data/profiles/&lt;slug&gt;/output/</code>
              so you can audit it post-apply.
            </li>
            <li>
              <strong>Selector breakage.</strong> Every ATS rebuilds its forms eventually. When that
              happens this script will start failing soft to <code class="font-mono">ManualApplyNeeded</code>.
              Inbox will surface the failures; you finish by hand until the adapter ships a fix.
            </li>
            <li>
      ...
      <Card.Root>
        ...
        <Card.Content class="space-y-3">
          <div class="flex items-center gap-1.5 text-xs flex-wrap">
            <span class="px-2 py-1 rounded bg-cyan-500/10 ring-1 ring-cyan-500/40 text-cyan-200 font-mono">Scored</span>
            <ArrowRight class="size-3 text-muted-foreground" />
            <span class="px-2 py-1 rounded bg-fuchsia-500/10 ring-1 ring-fuchsia-500/40 text-fuchsia-200 font-mono">Queued</span>
            <ArrowRight class="size-3 text-muted-foreground" />
            <span class="px-2 py-1 rounded bg-blue-500/10 ring-1 ring-blue-500/40 text-blue-200 font-mono">Applying</span>
            <ArrowRight class="size-3 text-muted-foreground" />
            <span class="px-2 py-1 rounded bg-violet-500/10 ring-1 ring-violet-500/40 text-violet-200 font-mono">Applied</span>
            <span class="text-muted-foreground/50">or</span>
            <span class="px-2 py-1 rounded bg-amber-500/10 ring-1 ring-amber-500/40 text-amber-200 font-mono">ManualApplyNeeded</span>
          </div>
          <ol class="text-xs text-muted-foreground space-y-1.5 list-decimal pl-5">
            <li>Click <strong>Apply</strong> on a Scored job (or auto-queue from CV generation) → status flips to <strong>Queued</strong>.</li>
            <li>The <code class="font-mono">apply-queue-drain</code> autopilot job runs daily on weekdays (or "Run drain now" from /queue) → status flips to <strong>Applying</strong>.</li>
            <li>Drain spawns the right per-portal Python adapter via <code class="font-mono">apply-portal.py</code>.</li>
            <li>Adapter fills the form, uploads the tailored CV, clicks Submit. Confirmation page → <strong>Applied</strong>.</li>
            <li>Any soft block (CAPTCHA, unknown field, anti-bot) → <strong>ManualApplyNeeded</strong> + Inbox issue with "Open posting" CTA.</li>
          </ol>
        </Card.Content>
      </Card.Root>
      ...
      <Card.Root>
        ...
        <Card.Content class="space-y-1.5">
          {#each PORTALS as p (p.id)}
            <div class="rounded-md border border-border/40 bg-card px-3 py-2 flex items-start gap-3">
              ...
              <div class="flex-1 min-w-0">
                <div class="text-sm font-medium flex items-center gap-2">
                  {p.label}
                  <span class={p.status === 'production' ? 'text-[10px] uppercase tracking-wider text-emerald-300/80' : 'text-[10px] uppercase tracking-wider text-amber-300/80'}>
                    {p.status}
                  </span>
                </div>
        ...
        <Card.Content class="space-y-3 text-sm leading-relaxed">
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-1">
              Default 4.0/5. Jobs below this threshold stay at <code class="font-mono">ManualApplyNeeded</code>
              even when autonomous mode is on — protecting recruiter time on borderline-fit roles. Tune via
              the slider on the /profile autonomous-apply card. Recommended floor: 4.0.
            </p>
          </div>
          <div>
            <h3 class="font-medium text-sm">Warmup days</h3>
            <p class="text-xs text-muted-foreground mt-1">
              For the first N days after enabling, the per-profile cap is clamped to 5 applications/day
              regardless of the global "Max applies / day" setting on /autopilot. Default 7 days.
              Buys you a window to confirm the pipeline works before opening the flood gates.
            </p>
          </div>
          <div>
            <h3 class="font-medium text-sm">Daily apply cap (global)</h3>
            <p class="text-xs text-muted-foreground mt-1">
              The <code class="font-mono">maxAppliesPerDay</code> threshold on /autopilot caps EVERY
              profile's autonomous applies combined. This is intentional — a single IP applying 60×/day
              across 3 profiles still trips LinkedIn's shadowban. The drain stops as soon as today's
              counter hits the cap.
            </p>
          </div>
        </Card.Content>
      </Card.Root>
          ...
          {#each FAILURE_MODES as f (f.id)}
            <div class="rounded-md border border-border/40 bg-card px-3 py-2 space-y-1">
              <div class="text-sm font-medium flex items-center gap-2">
                ...
                {f.summary}
                <span class="text-[10px] uppercase tracking-wider text-muted-foreground/60 font-mono">{f.id}</span>
              </div>
              <p class="text-[11px] text-muted-foreground/80 leading-relaxed">
                <strong class="text-foreground/80">When:</strong> {f.trigger}
              </p>
              <p class="text-[11px] text-muted-foreground/80 leading-relaxed">
                <strong class="text-foreground/80">Fix:</strong> {f.fix}
              </p>
            </div>
          {/each}
        </Card.Content>
      ...
      <Card.Root>
        ...
        <Card.Content class="space-y-3 text-sm">
          <ol class="list-decimal pl-5 space-y-1.5 text-muted-foreground">
            <li>Go to <a href="/profile" class="underline underline-offset-2 hover:text-foreground">/profile</a> → open the <strong>Autonomous apply</strong> card.</li>
            ...
            <li>The Apply button on every job in this profile now collapses to a single "Queue apply" click.</li>
          </ol>
          <p class="text-[11px] text-muted-foreground/70 pt-2 border-t border-border/30">
            <strong>To cancel an in-flight queue:</strong> on <a href="/queue" class="underline underline-offset-2 hover:text-foreground">/queue</a>, each Queued job has a <strong>Cancel</strong> button that reverts it to Scored. Once a job has transitioned to <strong>Applying</strong>, you cannot cancel — wait for the script to finish (or fail soft). Disable the toggle to stop future drains, but in-flight jobs will still complete their current run.
          </p>
        </Card.Content>
      </Card.Root>
