  ...
  const SOURCES: SourceDoc[] = [
    ...
    },
    {
      ...
      how: [
        ...
        'Generate an app password at myaccount.google.com/apppasswords (pick "Mail" + "Other → Heron")',
        'Optional: in Gmail, create a filter that auto-applies a label to LinkedIn/Indeed alert emails (e.g. "Heron/job-alerts")',
...
<div class="h-full overflow-y-auto">
  <Topbar
    title="Sources"
    subtitle="How each scanner works and when to reconnect"
    showTabs={false}
  />
    ...
    <div class="max-w-4xl mx-auto space-y-5">
      ...
      <div class="space-y-1.5 max-w-3xl">
        ...
        <p class="text-sm text-muted-foreground leading-relaxed">
          ...
          sources (LinkedIn, Indeed) need a one-time browser login. The Gmail IMAP source needs an
          app password. Always-on aggregators (ATS APIs, JobSpy, curated boards) run with no
          configuration. The <a
            href="/sources"
            class="underline underline-offset-2 hover:text-foreground">/sources</a
          >
      ...
      <Card.Root>
        <Card.Header>
          ...
          <Card.Description>
            Every source has a row in <code class="font-mono">data/sources.json</code> tracking its health.
            Each scan run calls one of two helpers:
          </Card.Description>
        </Card.Header>
        <Card.Content class="space-y-2">
          <ul
            class="text-[11.5px] text-muted-foreground/90 list-disc pl-5 leading-relaxed space-y-1"
          >
            <li>
              <strong>recordSuccess(id)</strong> — sets
              <code class="font-mono">connected: true</code>, updates
              <code class="font-mono">lastSuccessfulPullAt</code>, resets the failure counter.
            </li>
            <li>
          ...
          <div class={'rounded-lg border ' + tint(src.type)}>
            <div class="px-4 py-3 space-y-3">
              <div class="flex items-center gap-2">
                <span
                  class="size-7 rounded-md bg-background/60 ring-1 ring-border/40 flex items-center justify-center"
                >
                  <Icon class="size-3.5" />
                </span>
                <h3 class="text-sm font-semibold">{src.label}</h3>
                <span
                  class="ml-auto text-[9px] uppercase tracking-wider font-mono px-1.5 py-0.5 rounded border border-border/40 bg-background/50 text-muted-foreground"
                >
                  {src.type}
                </span>
              </div>
              ...
              <div class="text-[11.5px] space-y-1.5">
                <div>
                  ...
                  >
                  <ul
                    class="list-disc pl-5 mt-1 space-y-0.5 text-muted-foreground/90 leading-relaxed"
                  >
                    {#each src.how as h (h)}
                      <li>{h}</li>
                    {/each}
                ...
                <div>
                  ...
                  >
                  <ul
                    class="list-disc pl-5 mt-1 space-y-0.5 text-muted-foreground/80 leading-relaxed"
                  >
        ...
        <Card.Content class="space-y-2">
          <div class="space-y-2 text-[12px]">
            ...
            <div class="flex items-start gap-3">
              ...
              <p class="text-muted-foreground/90 leading-relaxed">
                Headless probe — checks the saved Playwright session is still valid, that IMAP login
                still works, or that the API key still authenticates. Updates
                <code class="font-mono">lastSuccessfulPullAt</code> on success.
              </p>
            </div>
            <div class="flex items-start gap-3">
              <span
                class="text-[11px] uppercase tracking-wider font-mono px-1.5 py-0.5 rounded border border-red-500/40 bg-red-500/10 text-red-700 dark:text-red-300 flex-shrink-0"
              >
      ...
      <Card.Root>
        ...
        <Card.Content class="space-y-2">
          <ul class="text-[12px] text-muted-foreground/90 leading-relaxed space-y-1.5">
            ...
            <li>
              <strong>Indeed captcha</strong> — Indeed periodically serves a "press and hold" test. The
              scraper exits cleanly with the message "captcha — re-login on /sources". Click Connect,
              complete the captcha + login, you're back. Indeed gets less aggressive over time once your
              IP has a stable history.
            </li>
            <li>
              <strong>Gmail "auth failed"</strong> — usually a revoked app password (Google revokes them
              when you change your account password) or 2FA was disabled. Generate a new app password
              and re-fill the form on /sources.
            </li>
            <li>
              <strong>"All scanners failed" on a daily run</strong> — typically a transient upstream issue
              (multiple ATS APIs returning 503 in the same window). Check the activity feed; if a single
              source is consistently failing, click Test on its card for a focused error.
            </li>
            <li>
              <strong>Anthropic 401 / Gemini 403</strong> — key was rotated upstream (e.g. you
              regenerated it from the Anthropic console but didn't update
              <code class="font-mono">.env</code>). Open Settings, paste the new key, save.
            </li>
          </ul>
        </Card.Content>
      </Card.Root>
