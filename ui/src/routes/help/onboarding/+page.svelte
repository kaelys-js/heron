...
<div class="h-full overflow-y-auto">
  <Topbar title="Onboarding" subtitle="What each wizard step does and how to redo it" showTabs={false} />
    ...
    <div class="max-w-4xl mx-auto space-y-5">
            ...
            <Card.Header>
              <div class="flex items-center justify-between gap-2">
                <div class="flex items-center gap-2">
                  <span class="size-7 rounded-md bg-muted/40 ring-1 ring-border/40 flex items-center justify-center">
                    <Icon class="size-3.5 text-muted-foreground" />
                  </span>
                  <Card.Title class="text-base">{i + 1}. {step.label}</Card.Title>
                </div>
                <a href={step.href} class="text-[11px] text-muted-foreground hover:text-foreground inline-flex items-center gap-1">
            ...
            <Card.Content class="pt-0 space-y-2">
              <div class="text-[11px] space-y-1">
                <div class="flex items-start gap-2">
                  <span class="text-[10px] uppercase tracking-wider font-mono px-1.5 py-0.5 rounded border border-emerald-500/40 bg-emerald-500/10 text-emerald-300 flex-shrink-0">writes</span>
                ...
                <div class="flex items-start gap-2">
                  <span class="text-[10px] uppercase tracking-wider font-mono px-1.5 py-0.5 rounded border border-blue-500/40 bg-blue-500/10 text-blue-300 flex-shrink-0">redo</span>
      ...
      <Card.Root>
        <Card.Header>
          ...
          <Card.Description>
            ...
            next page load. <strong>Your CV, profile, tracker, reports, and connected sources are
            not touched.</strong> Useful when you want to revisit the targeting / sources steps as
            a guided flow rather than from each individual page.
          </Card.Description>
        </Card.Header>
        <Card.Content class="space-y-2">
      ...
      <Card.Root>
        <Card.Header>
          ...
          <Card.Description>
            ...
            <code class="font-mono">profile.yml</code>, <code class="font-mono">portals.yml</code>, and
            <code class="font-mono">modes/_profile.md</code> manually (e.g. by editing files via your
            preferred editor + AI agent). Marks the wizard complete without writing any of those
            files. The system continues working as long as those files exist.
          </Card.Description>
        </Card.Header>
        <Card.Content>
          <ul class="text-[11px] text-muted-foreground/90 list-disc pl-5 space-y-0.5 leading-relaxed">
            <li><strong>Don't skip</strong> if any required file is missing — the daily scan will run but evaluations will fail.</li>
            <li><strong>Don't skip</strong> if you don't have an Anthropic key set — the deeper evaluation modes won't work.</li>
            <li>Skip <em>is</em> safe if you cloned an existing setup (dotfiles, prior career-ops install) and want to dismiss the redirect.</li>
          </ul>
        </Card.Content>
      </Card.Root>
      ...
      <Card.Root>
        ...
        <Card.Content class="space-y-1.5">
          <a href="/help/sources" class="flex items-center justify-between text-[12px] hover:text-foreground text-muted-foreground py-1">
