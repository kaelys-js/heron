    ...
    <div class="max-w-6xl mx-auto space-y-5">
      ...
      <div class="space-y-1.5 max-w-3xl">
        ...
        <p class="text-sm text-muted-foreground leading-relaxed">
          Each skill is a Claude Code slash-command that pairs a structured prompt with the data this dashboard
          maintains (CV, profile, pipeline, reports). Click a skill to read what it does, then copy the
          invocation to run it in your terminal.
        </p>
      </div>
        ...
        <div class="flex flex-wrap gap-1">
          {#each CATEGORIES as cat}
            ...
            >
              <CIcon class="size-3" />
              <span>{cat.label}</span>
              <span class={cn('text-[10px] tabular-nums', active ? 'opacity-70' : 'text-muted-foreground/60')}>{count}</span>
            </button>
          {/each}
        </div>
      </div>
            ...
            {#if items.length > 0}
              <section class="space-y-2.5">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-[10px] text-muted-foreground/60 tabular-nums">{items.length}</span>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2.5">
                  {#each items as s (s.id)}
                    ...
                    >
                      <Card.Root class="h-full hover:bg-accent/40 hover:border-accent/60 transition-colors">
                        <Card.Header class="pb-2">
                          <div class="flex items-start gap-3">
                            <div class="size-9 rounded-lg bg-muted/40 flex items-center justify-center text-base flex-shrink-0">
                              {s.emoji}
                            </div>
                            <div class="flex-1 min-w-0">
                              <div class="flex items-center gap-2 flex-wrap">
                                ...
                                {#if s.language === 'es'}
                                  <span class="text-[9px] font-mono uppercase text-muted-foreground/70 tracking-wider">es</span>
                                {/if}
                              </div>
                              {#if s.subtitle}
                                <Card.Description class="text-xs mt-0.5 leading-snug">{s.subtitle}</Card.Description>
                              {/if}
                            </div>
                            <ChevronRight class="size-3.5 text-muted-foreground/30 group-hover/skill:text-muted-foreground transition-colors flex-shrink-0 mt-1" />
                          </div>
                        </Card.Header>
                        <Card.Content class="pb-3 pt-0">
                          {#if s.description}
                            <p class="text-[11px] text-muted-foreground line-clamp-2 leading-relaxed">{s.description}</p>
                          {/if}
                          <div class="flex items-center gap-2 mt-2">
                            <code class="text-[10px] font-mono text-muted-foreground/70 truncate">{s.invocation}</code>
                          </div>
                        </Card.Content>
                      </Card.Root>
                    </button>
      ...
      <p class="text-[10px] text-muted-foreground/50 text-center pt-4">
        Skills live as markdown files in <code class="font-mono">modes/</code>. Edit any of them to customize the prompt;
        the dashboard hot-reloads on next page load.
      </p>
    </div>
  </div>
</div>
...
<!-- Detail sheet -->
<Sheet.Root open={openSkill != null} onOpenChange={(v: boolean) => { if (!v) openSkill = null; }}>
  <Sheet.Content side="right" class="w-full sm:max-w-2xl flex flex-col p-0 gap-0">
    {#if openSkill}
      {@const s = openSkill}
      <Sheet.Header class="px-6 pt-6 pb-4 border-b">
        <div class="flex items-start gap-3">
          <div class="size-12 rounded-xl bg-muted ring-1 ring-border/40 flex items-center justify-center text-2xl flex-shrink-0">
            {s.emoji}
          </div>
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center gap-2 mt-2 flex-wrap">
              <span class={cn('inline-flex items-center gap-1 text-[10px] uppercase tracking-wider font-medium')}>
              ...
              <span class="text-[10px] text-muted-foreground/60 tabular-nums">{(s.bytes / 1024).toFixed(1)} KB</span>
              {#if s.language === 'es'}
                <span class="text-[10px] text-muted-foreground/60">·</span>
      ...
      <div class="px-6 py-4 border-b space-y-4">
        ...
        <div class="space-y-2">
          <div class="flex items-baseline justify-between gap-2">
            ...
            <span class="text-[10px] text-muted-foreground/60">⌘ + paste in Claude Code, or terminal</span>
          </div>
          <div class="relative rounded-lg border border-border/60 bg-muted/40 overflow-hidden">
            <div class="flex items-center gap-2 px-3 py-3">
              ...
              <code class="flex-1 text-sm font-mono overflow-x-auto whitespace-nowrap text-foreground">{s.invocation}</code>
              <Button
                variant="ghost"
                size="sm"
            ...
            <div class="px-3 py-1.5 border-t border-border/40 bg-card/40 flex items-center gap-2 text-[10px] text-muted-foreground/80">
        ...
        {#if s.inputs && s.inputs.length > 0}
          <div>
            <div class="text-[10px] uppercase tracking-wider text-muted-foreground/80 font-medium mb-1.5">
