<script lang="ts">
  ...
  function leverageTint(kind: string): string {
    if (kind === 'multi-offer') return 'border-emerald-500/40 bg-emerald-500/10 text-emerald-200';
    if (kind === 'decision-deadline') return 'border-amber-500/40 bg-amber-500/10 text-amber-200';
...
<div class="mx-auto max-w-6xl space-y-6 p-6">
  ...
  <section class="rounded-lg border border-zinc-700 bg-zinc-900/40 p-5">
    ...
    {:else}
      <div class="grid grid-cols-2 gap-4 sm:grid-cols-5">
        <div>
          <div class="text-xs text-zinc-400">Applied</div>
          <div class="font-mono text-2xl">{data.funnel.applied}</div>
        </div>
        <div>
          <div class="text-xs text-zinc-400">Screened</div>
          <div class="font-mono text-2xl">{data.funnel.screened}</div>
          <div class="text-xs text-zinc-500">{pct(data.funnel.appliedToScreen)}</div>
        </div>
        <div>
          <div class="text-xs text-zinc-400">Interview</div>
          <div class="font-mono text-2xl">{data.funnel.interview}</div>
          <div class="text-xs text-zinc-500">{pct(data.funnel.screenToInterview)}</div>
        </div>
        <div>
          <div class="text-xs text-zinc-400">Offer</div>
          <div class="font-mono text-2xl">{data.funnel.offer}</div>
          <div class="text-xs text-zinc-500">{pct(data.funnel.interviewToOffer)}</div>
        </div>
        <div>
          <div class="text-xs text-zinc-400">Accepted</div>
          <div class="font-mono text-2xl">{data.funnel.accepted}</div>
          <div class="text-xs text-zinc-500">{pct(data.funnel.offerToAccept)}</div>
        </div>
      </div>
      {#if data.leakiest}
        <div class="mt-4 rounded border border-amber-500/30 bg-amber-500/5 p-3 text-sm">
          <span class="text-amber-200">Leakiest stage:</span>
          <span class="text-zinc-200">{data.leakiest.name}</span>
          <span class="text-zinc-400">({pct(data.leakiest.rate)})</span>
          <p class="mt-1 text-xs text-zinc-400">{data.advice}</p>
        </div>
      {/if}
    {/if}
  </section>
  ...
  <section class="rounded-lg border border-zinc-700 bg-zinc-900/40 p-5">
    ...
    {#if data.leverage.length === 0}
      <p class="text-sm text-zinc-500">No active leverage in flight.</p>
    {:else}
      <ul class="space-y-2">
  ...
  <section class="rounded-lg border border-zinc-700 bg-zinc-900/40 p-5">
    ...
    {#if data.hiddenCosts.length === 0}
      <p class="text-sm text-zinc-500">Nothing slipping — nice.</p>
    {:else}
      <ul class="space-y-2">
