    ...
    <div class="max-w-5xl mx-auto space-y-5">
      ...
      <div class="space-y-2">
        ...
        <p class="text-sm text-muted-foreground leading-relaxed">
          Plug in a structured offer (base, signing, target bonus, equity grant, benefits). Get the
          per-year breakdown, year-1 cash, 4-year nominal, 4-year discounted, and equity NPV. Toggle
          "Compare" to put a competing offer side-by-side for the negotiation conversation. All math
          is deterministic — no LLM eyeballing.
        </p>
      </div>
          ...
          <Card.Content class="space-y-3">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div class="space-y-1">
                ...
                <Input
                  type="number"
                  bind:value={offer.equity!.grantValueToday}
                  class="h-9 text-sm"
                />
              </div>
              <div class="space-y-1">
            ...
            <Card.Content class="space-y-3">
              <div class="grid grid-cols-2 gap-3">
                ...
                <div class="space-y-1">
                  ...
                  <Input
                    type="number"
                    bind:value={offerB.benefitsAnnualValue}
                    class="h-9 text-sm"
                  />
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <div class="space-y-1">
                  ...
                  <Input
                    type="number"
                    bind:value={offerB.equity!.grantValueToday}
                    class="h-9 text-sm"
                  />
                </div>
                <div class="space-y-1">
                  ...
                  <Input
                    type="number"
                    bind:value={offerB.equity!.growthRatePct}
                    class="h-9 text-sm"
                  />
                </div>
              </div>
        ...
        <Card.Root>
          ...
          <Card.Content class="space-y-4">
            ...
            <div class="text-xs text-muted-foreground">
              Effective annual: <span class="font-mono text-foreground"
                >{fmt(result.effectiveAnnual)}</span
              >
            </div>
      ...
      {#if compareMode && comparison}
        <Card.Root
          class={cn(
            comparison.preferred === 'tied'
              ? 'border-muted'
              : comparison.preferred === 'a'
            ...
            <div class="pt-2 border-t border-border/30 text-sm">
              {#if comparison.preferred === 'tied'}
                <span class="text-muted-foreground"
                  >Within $100 — practical tie. Other factors (commute, growth, culture) should
                  decide.</span
                >
              {:else}
                <span
      ...
      <div class="rounded-md border border-border/40 bg-muted/10 px-3 py-2 flex items-start gap-2">
        ...
        <p class="text-[11px] text-muted-foreground/80 leading-relaxed">
          Nominal numbers in your input currency, no inflation adjustment, no tax. For pre-IPO
          equity, set <em>Equity discount</em> generously (40-60%) — the 409A is almost always less than
          the eventual outcome but also less than the headline private valuation. Annual bonus is modeled
          at TARGET; companies hit 80-110% typically. Don't model refresh grants here — they're too speculative.
        </p>
      </div>
    </div>
  </div>
