    ...
    <div class="max-w-6xl mx-auto space-y-5">
      ...
      <div class="space-y-2">
        ...
        <p class="text-sm text-muted-foreground leading-relaxed">
          Live coaching while you code or whiteboard. Type the problem, draft a solution, click
          "Get coach feedback" — Claude gives you a 4-line scorecard: what's working, what to
          watch, what to do next, and a question a real interviewer would ask right now.
        </p>
      </div>

      <Tabs.Root value={activeTab} onValueChange={(v) => activeTab = v as 'code' | 'design'} class="w-full">
        <Tabs.List>
          <Tabs.Trigger value="code" class="gap-1.5"><Code class="size-3" /> Code</Tabs.Trigger>
          <Tabs.Trigger value="design" class="gap-1.5"><Network class="size-3" /> System design</Tabs.Trigger>
        </Tabs.List>
        ...
        <Tabs.Content value="code">
          <div class="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-4">
            <Card.Root>
              ...
              <Card.Content class="space-y-3">
                <div class="space-y-1">
                  ...
                  <Input id="code-problem" bind:value={codeProblem} placeholder="e.g. Implement an in-memory rate limiter, 100 req/min per user, sliding window" class="text-sm" />
                </div>
                <div class="space-y-1">
                ...
                <div class="flex items-center gap-2">
                  <Button onclick={() => requestFeedback('code')} disabled={codeBusy} class="gap-1.5">
                    {#if codeBusy}<Loader2 class="size-3 animate-spin" />{:else}<MessageSquare class="size-3" />{/if}
                    Get coach feedback
                  </Button>
                  <span class="text-[11px] text-muted-foreground">{codeFeedback.length} round{codeFeedback.length === 1 ? '' : 's'} this session</span>
                </div>
              </Card.Content>
            </Card.Root>
            ...
            <Card.Root>
              <Card.Header class="pb-2">
                <Card.Title class="text-sm flex items-center gap-1.5"><Sparkles class="size-3 text-amber-400" /> Coach feedback</Card.Title>
              </Card.Header>
              <Card.Content class="space-y-2">
                {#if codeFeedback.length === 0}
                  <p class="text-[11px] text-muted-foreground italic">Write code + click feedback to start.</p>
                {/if}
                {#each codeFeedback as f, i (i)}
                  <div class="rounded-md border border-border/40 bg-card px-2.5 py-2 space-y-1 text-[11px]">
                    <div class="text-[9px] uppercase tracking-wider text-muted-foreground">Round {i + 1}</div>
                    {#if f.working}<div><span class="text-emerald-300 font-mono text-[9px]">WORKING</span> <span class="text-muted-foreground">{f.working}</span></div>{/if}
                    {#if f.watch}<div><span class="text-amber-300 font-mono text-[9px]">WATCH</span> <span class="text-muted-foreground">{f.watch}</span></div>{/if}
                    {#if f.suggest}<div><span class="text-cyan-300 font-mono text-[9px]">SUGGEST</span> <span class="text-muted-foreground">{f.suggest}</span></div>{/if}
                    {#if f.question}<div><span class="text-fuchsia-300 font-mono text-[9px]">QUESTION</span> <span class="text-muted-foreground italic">"{f.question}"</span></div>{/if}
                  </div>
                {/each}
              </Card.Content>
            </Card.Root>
            ...
            <Card.Root>
              ...
              <Card.Content class="space-y-3">
                <div class="space-y-1">
                  ...
                  <Input id="design-problem" bind:value={designProblem} placeholder="e.g. Design a URL shortener for 1B daily clicks" class="text-sm" />
                </div>
                <div class="space-y-1">
                  ...
                  <div class="rounded-md border border-border/40 bg-card overflow-hidden">
                    ...
                    >
                      ...
                      {#each designEdges as edge (edge.from + edge.to)}
                        ...
                        {#if from && to}
                          <line x1={from.x + 50} y1={from.y + 20} x2={to.x + 50} y2={to.y + 20} stroke="#666" stroke-width="2" />
                          {#if edge.label}
                            <text x={(from.x + to.x) / 2 + 50} y={(from.y + to.y) / 2 + 12} fill="#999" font-size="10" text-anchor="middle">{edge.label}</text>
                          {/if}
                        {/if}
                      {/each}
                      <!-- Nodes -->
                      {#each designNodes as node (node.id)}
                        <g
                          ...
                          tabindex="0"
                        >
                          <rect width="100" height="40" rx="6" fill="#1f2937" stroke="#a78bfa" stroke-width="1.5" />
                          <text x="50" y="25" fill="#e9d5ff" font-size="12" text-anchor="middle">{node.label}</text>
                        </g>
                      {/each}
                    </svg>
                  </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                  <div class="space-y-1.5">
                    <div class="flex items-center justify-between">
                      ...
                      <Button size="sm" variant="ghost" onclick={addNode} class="h-6 text-[10px]"><Plus class="size-3" /></Button>
                    </div>
                    {#each designNodes as node, i (node.id)}
                      <div class="flex items-center gap-1">
                        <Input
                          type="text"
                          value={node.label}
                          oninput={(e) => { designNodes[i].label = (e.currentTarget as HTMLInputElement).value; designNodes = [...designNodes]; }}
                          class="h-7 text-xs"
                        />
                        <button onclick={() => removeNode(node.id)} class="text-muted-foreground hover:text-red-300" aria-label="Remove node"><X class="size-3" /></button>
                      </div>
                    {/each}
                  </div>
                  <div class="space-y-1.5">
                    <div class="flex items-center justify-between">
                      ...
                      <Button size="sm" variant="ghost" onclick={addEdge} class="h-6 text-[10px]"><Plus class="size-3" /></Button>
                    </div>
                    {#each designEdges as edge, i (i)}
                      <div class="flex items-center gap-1">
                        <select
                          value={edge.from}
                          onchange={(e) => { designEdges[i].from = (e.currentTarget as HTMLSelectElement).value; designEdges = [...designEdges]; }}
                        ...
                        <select
                          value={edge.to}
                          onchange={(e) => { designEdges[i].to = (e.currentTarget as HTMLSelectElement).value; designEdges = [...designEdges]; }}
                          class="h-7 rounded text-[10px] border border-border/40 bg-card flex-1"
                        >
                          {#each designNodes as n}<option value={n.id}>{n.label}</option>{/each}
                        </select>
                        <button onclick={() => removeEdge(i)} class="text-muted-foreground hover:text-red-300" aria-label="Remove edge"><X class="size-3" /></button>
                      </div>
                    {/each}
                  </div>
                </div>
                <div class="flex items-center gap-2">
                  <Button onclick={() => requestFeedback('design')} disabled={designBusy} class="gap-1.5">
                    {#if designBusy}<Loader2 class="size-3 animate-spin" />{:else}<MessageSquare class="size-3" />{/if}
                    Get coach feedback
                  </Button>
                  <span class="text-[11px] text-muted-foreground">{designFeedback.length} round{designFeedback.length === 1 ? '' : 's'}</span>
                </div>
              </Card.Content>
            </Card.Root>
            ...
            <Card.Root>
              ...
              <Card.Content class="space-y-2">
                {#if designFeedback.length === 0}
                  <p class="text-[11px] text-muted-foreground italic">Draft a diagram + click feedback to start.</p>
                {/if}
      ...
      <!-- Back -->
      <div class="pt-2">
        <Button variant="ghost" size="sm" href={'/job/' + (data.job?.id ?? '')} class="text-xs gap-1">
