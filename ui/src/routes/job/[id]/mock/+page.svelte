...
<div class="h-full overflow-y-auto">
  <Topbar title="Mock interview · {data.job?.role ?? 'Job'}" subtitle={data.job?.company} showTabs={false} breadcrumb="Job" breadcrumbHref={'/job/' + (data.job?.id ?? '')} />
  ...
  <div class="p-6 pb-24">
    <div class="max-w-3xl mx-auto space-y-5">
      ...
      {#if !recognitionSupported || !synthesisSupported}
        <div class="rounded-md border border-amber-500/40 bg-amber-500/5 px-3 py-2 flex items-start gap-2">
          <AlertCircle class="size-4 text-amber-400 mt-0.5 flex-shrink-0" />
          <div class="text-xs text-amber-200/90 leading-relaxed">
            {#if !recognitionSupported}
              <p><strong>Speech recognition not supported.</strong> Chrome / Edge / Brave have it; Firefox + Safari don't. You can still type answers manually below.</p>
            {/if}
        ...
        <Card.Content class="space-y-2">
          <div class="grid grid-cols-2 sm:grid-cols-5 gap-2">
            {#each stageOptions as opt}
              <button
                type="button"
                onclick={() => { if (!sessionActive) stage = opt.id; }}
                class={cn(
                  'rounded-md border px-2.5 py-2 text-left transition',
                  stage === opt.id ? 'border-orange-500/60 bg-orange-500/10 text-orange-100' : 'border-border/40 bg-card hover:border-border',
      ...
      <div class="flex items-center gap-2 flex-wrap">
        {#if !sessionActive}
          <Button onclick={beginSession} disabled={waitingForServer} class="gap-1.5">
            {#if waitingForServer}<Loader2 class="size-3.5 animate-spin" />{:else}<Play class="size-3.5" />{/if}
            Start session · {stage}
          </Button>
        {:else}
          <Button onclick={endSession} variant="destructive" disabled={waitingForServer} class="gap-1.5">
        ...
        <Card.Root>
          <Card.Header class="pb-2">
            <Card.Title class="text-sm flex items-center gap-2">
              ...
              {#if speaking}<span class="text-[10px] text-orange-300 animate-pulse">speaking…</span>{/if}
            </Card.Title>
          </Card.Header>
          <Card.Content class="space-y-3">
            ...
            <div class="space-y-1.5">
              <Label class="text-xs">Your answer ({listening ? 'listening — speak now' : 'type or click Speak'})</Label>
              <Textarea bind:value={currentAnswer} rows={4} placeholder="Speak your answer or type here…" class="text-sm" />
            </div>
            ...
            <div class="flex items-center gap-2 flex-wrap">
              {#if recognitionSupported}
                ...
                {/if}
              {/if}
              <Button onclick={submitAnswer} disabled={!currentAnswer.trim() || waitingForServer} class="gap-1.5">
                {#if waitingForServer}
            ...
            {#if lastFeedback}
              <div class="rounded-md border border-border/40 bg-muted/20 px-3 py-2 text-xs space-y-1">
                <div class="flex items-center gap-2">
                  <span class="text-muted-foreground">Last turn:</span>
                  {#if lastScore != null}
                    <span class={cn(
                      'font-mono px-1.5 py-0.5 rounded border text-[10px]',
                      lastScore >= 4 ? 'border-emerald-500/40 text-emerald-300 bg-emerald-500/10' :
                      lastScore >= 3 ? 'border-amber-500/40 text-amber-300 bg-amber-500/10' :
                  ...
                  <div class="flex items-start gap-2 text-[10px]">
                    {#if t.score != null}
                      <span class={cn(
                        ...
                        t.score >= 4 ? 'border-emerald-500/40 text-emerald-300' :
                        t.score >= 3 ? 'border-amber-500/40 text-amber-300' :
      ...
      {#if endedSummary}
        <Card.Root class="border-emerald-500/40 bg-emerald-500/5">
          ...
          <Card.Content class="space-y-2">
            <article class="prose prose-invert prose-sm max-w-none whitespace-pre-wrap font-sans text-sm">{endedSummary}</article>
            {#if endedTranscriptPath}
              <p class="text-[10px] text-muted-foreground/70 font-mono pt-2 border-t border-border/30">
