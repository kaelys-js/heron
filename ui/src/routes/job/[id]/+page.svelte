...
<div class="h-full overflow-y-auto">
  <Topbar
    title={data.job?.role ?? 'Job'}
    subtitle={data.job?.company}
    breadcrumb="Pipeline"
    breadcrumbHref="/pipeline"
    showTabs={false}
  />
  <div class="flex">
      ...
      <div class="max-w-3xl mx-auto px-8 py-6 space-y-6">
        {#if data.job}
          ...
          >
            ...
            -->
            <div
              class="mb-4 -mx-1 px-1 overflow-x-auto [&::-webkit-scrollbar]:hidden [scrollbar-width:none]"
            >
            ...
            <Tabs.Content value="overview">
              {#if data.report}
                <details class="group" open={!data.summary}>
                  <summary
                    class="text-[11px] uppercase tracking-wider text-muted-foreground/70 cursor-pointer hover:text-foreground select-none mb-3 flex items-center gap-1.5 list-none"
                  >
                    <ChevronDown
                      class="size-3.5 transition-transform group-open:rotate-0 -rotate-90"
                    />
                    <FileText class="size-3" />
                    Full evaluation report
                    <span class="text-muted-foreground/50"
                      >· {data.report.length.toLocaleString()} chars</span
                    >
                  </summary>
                  <article
                    class="prose prose-invert prose-sm max-w-none prose-headings:font-semibold prose-headings:tracking-tight prose-pre:bg-muted prose-table:text-sm prose-th:font-semibold prose-strong:text-foreground prose-a:text-foreground prose-a:underline prose-a:underline-offset-2"
                  >
                    {@html html}
                  </article>
                </details>
            ...
            <Tabs.Content value="prep">
              ...
              {:else if !prepContent}
                <div class="space-y-3">
                  <Button
                    onclick={loadPrep}
                    disabled={prepLoading || !data.job.reportFile}
                    class="gap-1.5"
                  >
                    {#if prepLoading}<Loader2 class="size-3.5 animate-spin" /> Generating…{:else}<Briefcase
                        class="size-3.5"
                      /> Generate Interview Prep Brief{/if}
                  </Button>
                  <p class="text-xs text-muted-foreground leading-relaxed max-w-2xl">
                    Produces 8–12 likely questions, a STAR map mapping your proof points to expected
                    behaviorals, a 5-topic study plan, 3 strong talking points, red flags to watch
                    for, and 5 questions to ask back. Auto-fires when this job's status flips to <span
                      class="font-mono">Interview</span
                    > — so by the time you have an interview scheduled, the brief is already waiting.
                  </p>
                </div>
              {:else}
                <div class="space-y-4">
                  <div class="flex items-center gap-2">
                    ...
                    >
                      {#if prepLoading}<Loader2 class="size-3 animate-spin" /> Regenerating…{:else}<Briefcase
                          class="size-3"
                        /> Regenerate{/if}
                    </Button>
                  </div>
            ...
            <Tabs.Content value="mock">
              <div class="flex items-center gap-2 mb-3">
                <select
                  bind:value={chatPersona}
                  class="text-xs h-8 px-2 rounded border bg-background"
                >
              ...
              <div
                class="border rounded-lg bg-muted/20 p-4 max-h-[500px] overflow-y-auto space-y-4"
              >
                ...
                {#if chatHistory.length === 0}
                  <p class="text-sm text-muted-foreground italic">
                    Click Start Mock — interviewer will open with a question.
                  </p>
                {/if}
                {#if chatLoading}
            ...
            <Tabs.Content value="negotiation">
              <div class="space-y-3">
                <p class="text-sm text-muted-foreground">
                  Paste offer details (base / equity / bonus / location / start date / competing
                  offers):
                </p>
                <Textarea
                  bind:value={offerInput}
                  rows={6}
                  placeholder="Base: $X / Equity: ... / etc."
                  class="font-mono text-sm"
                />
                <Button
                  onclick={getNegotiation}
                  disabled={negotiationLoading || !offerInput.trim()}
                >
              ...
              <div class="space-y-4">
                <div class="flex items-center gap-2 flex-wrap">
                  ...
                  <Button
                    ...
                    disabled={outreachBusy}>Hiring manager</Button
                  >
                  <Button
                    ...
                    disabled={outreachBusy}>Recruiter</Button
                  >
                  <Button
                    ...
                    disabled={outreachBusy}>Peer at the company</Button
                  >
                ...
                <p class="text-[11px] text-muted-foreground/80 leading-relaxed">
                  Generates short LinkedIn messages tuned for {outreachPersona === 'hiring-manager'
                    ? 'an engineering manager or other decision-maker'
                    : outreachPersona === 'recruiter'
                      ? 'an in-house recruiter or sourcer'
                      : 'a current employee on the team (a peer, not someone with hiring power)'}.
                  Each variant pulls one specific proof point from your CV and references something
                  concrete from the job's evaluation report — so it doesn't read like the generic
                  "I'm interested in your role" template everyone else sends.
                </p>
                ...
                {#if outreachContent}
                  ...
                  <div class="flex items-center gap-2 pt-2 border-t border-border/40">
                    ...
                    >
                      Search LinkedIn for {outreachPersona === 'hiring-manager'
                        ? 'managers'
                        : outreachPersona === 'recruiter'
                          ? 'recruiters'
                          : 'team members'} at {data.job.company}
            ...
            <Tabs.Content value="cover-letter">
              <div class="space-y-3 max-w-full overflow-hidden">
                ...
                <div class="flex items-start justify-between gap-3 flex-wrap">
                  <div class="space-y-1 max-w-2xl">
                    ...
                    <p class="text-[11px] text-muted-foreground leading-relaxed">
                      ...
                      your strongest CV proof points are matched to the most important requirements
                      in the JD, then tied to a public detail about the company. The output is plain
                      markdown — copy it into the application form, an email, or a Word doc.
                    </p>
                  </div>
                ...
                {:else if coverContent}
                  ...
                  <article
                    class="prose prose-invert prose-sm max-w-full overflow-hidden rounded-md border border-border/40 bg-card px-4 py-3 prose-headings:font-semibold prose-pre:bg-muted prose-pre:whitespace-pre-wrap prose-pre:break-words prose-code:break-words prose-strong:text-foreground prose-a:break-all"
                  >
                    {@html coverHtml}
                  </article>
                  ...
                  <div class="space-y-3">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div class="rounded-md border border-border/40 bg-card px-3 py-2.5 space-y-1">
                        ...
                        <ul
                          class="text-[11px] text-muted-foreground/90 leading-relaxed list-disc pl-4 space-y-0.5"
                        >
                          <li>Single page, ≤ 350 words</li>
                          <li>
                            Your two strongest CV proof points (with numbers, not invented ones)
                          </li>
                      ...
                      <div class="rounded-md border border-border/40 bg-card px-3 py-2.5 space-y-1">
                        ...
                        >
                          <li>
                            Reading it. Don't send something Claude wrote without scanning it first.
                          </li>
                          <li>
                            Picking the medium (paste into the portal, an email, or a Word doc)
                          </li>
                          <li>Submitting — this never sends anything for you</li>
                          <li>
                            Filling any <code class="font-mono">_TODO_</code> markers if profile.yml is
                            incomplete
                          </li>
                        </ul>
                      </div>
