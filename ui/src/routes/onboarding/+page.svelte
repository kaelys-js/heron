  ...
  {#if isNewProfile}
    ...
    <div class="space-y-4 rounded-md border border-blue-500/30 bg-blue-500/5 p-4">
      ...
      <div class="space-y-1.5">
        ...
        <p class="text-[11px] text-muted-foreground/80">
          Used for the profile-switcher dot in the sidebar + the badge on each job in cross-profile
          views.
        </p>
      </div>
  ...
  {#if !isNewProfile}
      ...
      <div class="space-y-1.5">
        {#each STEPS as s, i (s.label)}
          ...
          <div class="flex items-start gap-3 px-3 py-2 rounded-md border border-border/40 bg-card">
            <div
              class="size-7 rounded-md bg-muted/40 ring-1 ring-border/40 flex items-center justify-center flex-shrink-0 mt-0.5"
            >
    ...
    <div class="rounded-md border border-border/40 bg-muted/20 px-4 py-3 space-y-1">
      ...
      <ul class="text-[11px] text-muted-foreground/90 list-disc pl-4 leading-relaxed space-y-0.5">
        <li>
          A daily scan that pulls from your authenticated LinkedIn + Indeed + Gmail alerts + 9 ATS
          providers + JobSpy aggregators
        </li>
        <li>
          Personalized scoring for every job (Gemini first-pass, Claude deep-eval on the high-fit
          ones)
        </li>
        <li>One-click tailored CV PDF + cover letter per job that scores ≥4/5</li>
        <li>
          Status tracking from Discovered → Ready → Applied → Interview → Offer (or Rejected, with
          patterns analysis)
        </li>
      </ul>
    </div>
