...
<main class="mx-auto flex min-h-screen max-w-xl flex-col justify-center p-8">
  <header class="mb-8">
    ...
    <p class="mt-2 text-sm text-muted-foreground">
      {#if data.isFirstUser}
        The first account you create becomes the owner of this install. You can
        invite others later from <code class="font-mono text-xs">/settings/users</code>.
      {:else}
        Sign in to continue setup, or create a new account if someone shared an
  ...
  <section class="grid gap-3">
    {#if data.isFirstUser}
      ...
      {#if data.githubEnabled}
        <Button href="/api/auth/sign-in/social?provider=github" variant="outline" class="justify-start gap-3">
          <Code class="h-4 w-4" />
  ...
  <p class="mt-8 text-xs text-muted-foreground">
    Career-ops stores your data locally. The owner account can invite household
    members or teammates with single-use codes that expire after 30 minutes.
  </p>
</main>
