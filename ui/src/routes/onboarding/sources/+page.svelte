...
<div class="space-y-6">
  <header class="space-y-2">
    ...
    <p class="text-sm text-muted-foreground leading-relaxed max-w-xl">
      Connect the personal sources for fuller coverage. LinkedIn + Indeed scrape your <em>logged-in</em>
      feed via a saved Playwright browser session — same view you'd see scrolling LinkedIn yourself,
      no API keys, no OAuth. Gmail polls a label for job-alert emails. All three are optional —
      skip any and the daily scan still runs against the always-on aggregators.
    </p>
  </header>
  ...
  {#if linkedin}
    ...
    <div class={cn('rounded-md border px-4 py-3 transition-colors', statusTint(row.state))}>
      <div class="flex items-start gap-3">
        <div class="size-9 rounded-md bg-blue-500/10 ring-1 ring-blue-500/30 flex items-center justify-center flex-shrink-0">
          <Briefcase class="size-4 text-blue-400" />
        </div>
        <div class="flex-1 min-w-0 space-y-1">
          <div class="flex items-center gap-2 flex-wrap">
            <h3 class="text-sm font-semibold">LinkedIn</h3>
            <span class="text-[9px] uppercase tracking-wider font-mono px-1.5 py-0.5 rounded border border-emerald-500/40 bg-emerald-500/10 text-emerald-300">recommended</span>
          </div>
      ...
      {#if row.state.lastError && !row.state.connected}
        <div class="rounded-md border border-red-500/30 bg-red-500/5 px-2.5 py-1.5 mt-3 flex items-start gap-2">
  ...
  {#if indeed}
    ...
    <div class={cn('rounded-md border px-4 py-3 transition-colors', statusTint(row.state))}>
      <div class="flex items-start gap-3">
        <div class="size-9 rounded-md bg-indigo-500/10 ring-1 ring-indigo-500/30 flex items-center justify-center flex-shrink-0">
  ...
  {#if gmail}
    ...
    {@const status = statusLabel(row.state)}
    {@const b = busy[row.id]}
    <div class={cn('rounded-md border px-4 py-3 transition-colors space-y-3', statusTint(row.state))}>
      <div class="flex items-start gap-3">
        <div class="size-9 rounded-md bg-rose-500/10 ring-1 ring-rose-500/30 flex items-center justify-center flex-shrink-0">
          <Mail class="size-4 text-rose-400" />
        </div>
        <div class="flex-1 min-w-0 space-y-1">
          <div class="flex items-center gap-2 flex-wrap">
            <h3 class="text-sm font-semibold">Gmail (job alerts)</h3>
            <span class="text-[9px] uppercase tracking-wider font-mono px-1.5 py-0.5 rounded border border-zinc-500/40 bg-zinc-500/10 text-muted-foreground">optional</span>
          </div>
          <p class="text-[11px] text-muted-foreground leading-relaxed">
        ...
        <div class="flex items-center gap-1.5 flex-shrink-0">
          ...
          {:else}
            ...
            >
              {#if b === 'connect'}<Loader2 class="size-3 animate-spin" /> Testing…{:else}<Plug class="size-3" /> Test & save{/if}
            </Button>
          {/if}
        </div>
      </div>
      ...
      {#if !row.state.connected}
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-2 border-t border-border/40">
          ...
          <div>
            ...
            <Input id="gmail-label" bind:value={gmailForm.label} class="h-8 text-xs" placeholder="INBOX or career-ops/job-alerts" />
          </div>
          <div>
        ...
        >
          <span class="inline-flex items-center gap-1">
            <ChevronDown class={cn('size-3 transition-transform', appPasswordHelp && 'rotate-180')} />
            How do I get an app password?
          </span>
          <a href="https://myaccount.google.com/apppasswords" target="_blank" rel="noopener" class="inline-flex items-center gap-1 hover:text-foreground" onclick={(e) => e.stopPropagation()}>
            Open in Google <ExternalLink class="size-2.5" />
          </a>
        </button>
        {#if appPasswordHelp}
          <ol class="list-decimal pl-5 text-[11px] text-muted-foreground/90 space-y-1 leading-relaxed">
            <li>Enable 2-factor auth on the Google account if you haven't already (required for app passwords).</li>
            <li>Visit <a href="https://myaccount.google.com/apppasswords" target="_blank" rel="noopener" class="underline underline-offset-2 hover:text-foreground">myaccount.google.com/apppasswords</a>. Pick "Mail" + "Other (custom name)" → "career-ops".</li>
            <li>Copy the 16-character password and paste it above. Stored locally in <code class="font-mono">.env</code>; never sent anywhere except your own Gmail server over TLS.</li>
            <li>Optional: in Gmail create a filter that auto-applies a label to LinkedIn/Indeed alert emails (e.g. "career-ops/job-alerts"), then point this card at that label instead of INBOX.</li>
          </ol>
        {/if}
      {/if}
    </div>
  ...
  {#if alwaysOn.length > 0}
    <div class="rounded-md border border-border/40 bg-muted/20 px-4 py-3 space-y-2">
      ...
      <p class="text-[11px] text-muted-foreground leading-relaxed">
        These run regardless — listed here so you know what's covered. The daily fan-out hits all of them.
      </p>
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-1">
        {#each alwaysOn as src (src.id)}
          <div class="rounded border border-border/40 bg-card px-2.5 py-1.5">
            ...
            <p class="text-[10px] text-muted-foreground/80 leading-relaxed mt-0.5 line-clamp-2">{src.description}</p>
          </div>
        {/each}
      </div>
    </div>
  ...
  <div class="flex items-center justify-between pt-4 border-t border-border/40">
    <a href="/onboarding/targeting" class="text-xs text-muted-foreground hover:text-foreground inline-flex items-center gap-1">
