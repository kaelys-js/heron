...
<div class="space-y-6">
  <header class="space-y-2">
    ...
    <p class="text-sm text-muted-foreground leading-relaxed max-w-xl">
      ...
      Claude does the deep dive (saves a lot of money). Adzuna is a niche aggregator, only enable
  ...
  <div class="space-y-4">
    ...
    <div class="rounded-md border border-amber-500/40 bg-amber-500/5 px-4 py-3 space-y-2">
      <div class="flex items-center justify-between">
        ...
        <a href="https://console.anthropic.com/settings/keys" target="_blank" rel="noopener" class="text-[11px] text-muted-foreground hover:text-foreground inline-flex items-center gap-1">
          Get a key <ExternalLink class="size-2.5" />
        </a>
      </div>
      <p class="text-[11px] text-muted-foreground">Powers deep evaluations + agent chat. Starts with <code class="font-mono">sk-ant-api03-</code>.</p>
      <div class="flex items-center gap-2">
        <Input
          id="anthropic"
          type="password"
          placeholder={isSet('ANTHROPIC_API_KEY') ? data.masked.ANTHROPIC_API_KEY : 'sk-ant-api03-…'}
          bind:value={pending.ANTHROPIC_API_KEY}
          class="font-mono text-xs"
        />
        {#if isSet('ANTHROPIC_API_KEY') || pending.ANTHROPIC_API_KEY}
          <Button variant="outline" size="sm" class="h-9 gap-1.5 flex-shrink-0" onclick={() => probe('anthropic')} disabled={probing.anthropic}>
      ...
      {#if probeResult.anthropic}
        ...
        <div class={r.ok ? 'text-[11px] text-emerald-400 inline-flex items-center gap-1' : 'text-[11px] text-red-400 inline-flex items-center gap-1'}>
    ...
    <div class="rounded-md border border-border/40 bg-card px-4 py-3 space-y-2">
      <div class="flex items-center justify-between">
        <Label for="gemini" class="text-sm font-medium">Gemini <span class="text-[10px] text-muted-foreground/70">(recommended)</span></Label>
        <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener" class="text-[11px] text-muted-foreground hover:text-foreground inline-flex items-center gap-1">
          Free key <ExternalLink class="size-2.5" />
        </a>
      </div>
      <p class="text-[11px] text-muted-foreground">First-pass scoring on the daily ~30 new postings. Free tier (1M tokens/day) is more than enough. Starts with <code class="font-mono">AIza</code>.</p>
      <div class="flex items-center gap-2">
        <Input
          id="gemini"
          type="password"
        ...
        {#if isSet('GEMINI_API_KEY') || pending.GEMINI_API_KEY}
          <Button variant="outline" size="sm" class="h-9 gap-1.5 flex-shrink-0" onclick={() => probe('gemini')} disabled={probing.gemini}>
    ...
    <details class="rounded-md border border-border/40 bg-card px-4 py-3 group">
      ...
      <div class="mt-3 space-y-2">
        <p class="text-[11px] text-muted-foreground">Adds Adzuna to the JobSpy aggregator list. Only fill in if you specifically want Adzuna's coverage.</p>
        <div class="grid grid-cols-2 gap-2">
          <div>
            <Label for="adzuna-id" class="text-xs">App ID</Label>
            <Input id="adzuna-id" type="password" bind:value={pending.ADZUNA_APP_ID} class="font-mono text-xs" placeholder={isSet('ADZUNA_APP_ID') ? data.masked.ADZUNA_APP_ID : '8 hex chars'} />
          </div>
          <div>
            <Label for="adzuna-key" class="text-xs">App Key</Label>
            <Input id="adzuna-key" type="password" bind:value={pending.ADZUNA_APP_KEY} class="font-mono text-xs" placeholder={isSet('ADZUNA_APP_KEY') ? data.masked.ADZUNA_APP_KEY : '32 hex chars'} />
          </div>
        </div>
      </div>
    </details>
