  ...
  async function applyOne(s: Suggestion) {
    ...
    if (s.op === 'manual') {
      toast.info('Manual recommendation', {
        description:
          'Read the suggestion and edit the file yourself. ' + (s.targetFiles?.[0] ?? ''),
      });
      return;
    }
    applyingId = s.id;
    ...
    <div class="max-w-4xl mx-auto space-y-5">
      ...
      <div class="space-y-2">
        ...
        <p class="text-sm text-muted-foreground leading-relaxed">
          ...
          changes to portals.yml / _profile.md / automation thresholds that should improve your hit
          rate. Each suggestion is one-click-applyable; a <code class="font-mono text-[11px]"
            >.bak</code
          >
      ...
      {#if data.analysis?.metadata?.total != null}
        <div
          class="rounded-md border border-border/40 bg-card px-3 py-2 flex items-center gap-4 flex-wrap text-xs"
        >
          <span>
          ...
          {#if data.analysis.scoreThreshold?.recommended}
            <span
              >· Recommended min score: <span class="font-mono"
                >{data.analysis.scoreThreshold.recommended}/5</span
              ></span
            >
          {/if}
        </div>
      ...
      {#if data.suggestions.length === 0}
        <div class="rounded-md border border-dashed border-border/40 px-3 py-8 text-center">
          ...
          <p class="text-sm text-muted-foreground">
            No structured suggestions yet. You need a tracker with at least 2-3 applications + their
            reports for the analyzer to find patterns.
          </p>
        </div>
      {:else}
        <div class="space-y-2">
      ...
      <div class="rounded-md border border-border/40 bg-muted/10 px-3 py-2 flex items-start gap-2">
        ...
        <p class="text-[11px] text-muted-foreground/80 leading-relaxed">
          ...
          modifying. To revert: <code class="font-mono">mv portals.yml.bak portals.yml</code>. The
          analyzer runs fresh on every page load — re-applying the same suggestion is a no-op when
