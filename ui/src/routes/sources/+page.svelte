    ...
    <div class="max-w-5xl mx-auto space-y-5">
      ...
      <div class="space-y-1.5 max-w-3xl">
        ...
        <p class="text-sm text-muted-foreground leading-relaxed">
          ...
          session in a persistent Playwright profile so we see the same personalized feed you'd see
          when scrolling LinkedIn yourself. Email ingestion polls a Gmail label via IMAP. The
        ...
        {#each data.sources as row (row.id)}
          ...
          <Card.Root class={cn('transition-colors', statusTint(row.state))}>
            <Card.Content class="px-4 py-3 flex items-start gap-3">
              <div
                class="size-8 rounded-md bg-muted/40 ring-1 ring-border/40 flex items-center justify-center flex-shrink-0 mt-0.5"
              >
                ...
                <div class="flex items-center gap-2 text-[11px] pt-1">
                  ...
                  {#if row.pulls.last7d > 0 || row.pulls.total > 0}
                    ...
                    <span class="text-muted-foreground/80 font-mono">
                      {row.pulls.last7d.toLocaleString()} this week · {row.pulls.total.toLocaleString()}
                      total
                    </span>
                  {/if}
                </div>
              ...
              <div class="flex items-center gap-1.5 flex-shrink-0">
                ...
                {:else if row.authKind === 'env-key'}
                  ...
                  >
                    {#if b === 'test'}<Loader2 class="size-3 animate-spin" />{:else}<RefreshCw
                        class="size-3"
                      />{/if}
                    Test
                  </Button>
            ...
            {#if row.id === 'gmail-imap' && !row.state.connected}
              <Card.Content class="px-4 pb-3 pt-0 border-t border-border/40">
                ...
                <p class="text-[11px] text-muted-foreground/80 mt-2 leading-relaxed">
                  ...
                  >
                  — needs 2FA enabled on the account first. Stored locally in
                  <code class="font-mono">.env</code>; never sent anywhere except your own Gmail
                  server over TLS.
                </p>
              </Card.Content>
            {/if}
