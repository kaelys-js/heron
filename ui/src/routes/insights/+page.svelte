    ...
    <div class="max-w-5xl mx-auto space-y-5">
      ...
      <div class="flex items-start justify-between gap-4">
        <div class="space-y-1.5 max-w-3xl">
          ...
          <p class="text-sm text-muted-foreground leading-relaxed">
            Looks at every application in your tracker plus every deep-evaluation report you've generated, then
            reports back where you're winning, where the system is wasting your effort, and what to change. The
            answers get more reliable the more <em>finished</em> outcomes you have — applications still in
            "Applied" don't count yet.
          </p>
        </div>
      ...
      {:else if finishedOutcomes < 5}
        <EmptyState
          ...
          description={'You have ' + total + ' applications tracked but only ' +
            finishedOutcomes + ' have a final outcome (Rejected, Interview/Offer, or SKIP). Patterns need at least 5 finished outcomes — preferably 10+ — before they\'re trustworthy.'}
        />
      {:else}
          ...
          <Card.Root>
            <Card.Header class="pb-2">
              ...
              <Card.Description class="text-xs">
                How your {fmtCount(total)} applications break down today. "Self-filtered" = SKIP rows you
                discarded before applying; "Pending" = no final answer yet.
              </Card.Description>
            </Card.Header>
            <Card.Content>
              <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div class="rounded-md border border-emerald-500/30 bg-emerald-500/5 px-3 py-2.5">
                  <div class="text-[10px] uppercase tracking-wider text-emerald-300/80 flex items-center gap-1">
                    <CheckCircle2 class="size-3" /> Positive
                  </div>
                  <div class="text-lg font-semibold tabular-nums text-emerald-200 mt-0.5">{fmtCount(outcomes.positive)}</div>
                ...
                <div class="rounded-md border border-red-500/30 bg-red-500/5 px-3 py-2.5">
                  ...
                  <div class="text-lg font-semibold tabular-nums text-red-200 mt-0.5">{fmtCount(outcomes.negative)}</div>
                ...
                <div class="rounded-md border border-zinc-500/30 bg-zinc-500/5 px-3 py-2.5">
                  <div class="text-[10px] uppercase tracking-wider text-zinc-300/80">Self-filtered</div>
                  <div class="text-lg font-semibold tabular-nums text-zinc-200 mt-0.5">{fmtCount(outcomes.self_filtered)}</div>
                ...
                <div class="rounded-md border border-blue-500/30 bg-blue-500/5 px-3 py-2.5">
                  ...
                  <div class="text-lg font-semibold tabular-nums text-blue-200 mt-0.5">{fmtCount(outcomes.pending)}</div>
        ...
        {#if p.recommendations && p.recommendations.length > 0}
          <Card.Root class="border-amber-500/40 bg-amber-500/5">
            <Card.Header class="pb-3">
              ...
              <Card.Description class="text-xs">
                Concrete actions auto-derived from your conversion data. Each one cites the evidence — sanity-check
                the reasoning before you change a filter or mode.
              </Card.Description>
            </Card.Header>
            <Card.Content class="space-y-2.5">
              {#each p.recommendations as rec, i (i)}
                <div class="rounded-md border border-amber-500/20 bg-background/40 px-3 py-2.5 space-y-1">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
          <Card.Root>
            <Card.Header class="pb-2">
              ...
              <Card.Description class="text-xs">
                Counts at each stage of your tracker. Wide bars at the top, narrow bars at the bottom is the
                normal shape — a wide "evaluated" with no movement past it is a sign the funnel is leaking.
              </Card.Description>
            </Card.Header>
            <Card.Content class="space-y-1.5">
              {#each funnelRows as [stage, count]}
                {@const w = funnelTotal > 0 ? Math.max(0, Math.min(100, (count / funnelTotal) * 100)) : 0}
          ...
          <Card.Root>
            <Card.Header class="pb-2">
              ...
              <Card.Description class="text-xs">
                Average fit score (Bloque A in each report, 0–5) grouped by what eventually happened. If
                "positive" averages much higher than "negative", your scoring is calibrated; if not, the
                threshold below has more weight.
              </Card.Description>
            </Card.Header>
            <Card.Content class="space-y-1.5">
              {#each Object.entries(p.scoreComparison ?? {}) as [group, stats]}
                {@const tint = group === 'positive' ? 'text-emerald-300'
                  : group === 'negative' ? 'text-red-300'
                ...
                <div class="flex items-center gap-3 text-xs">
                  ...
                  <div class="w-12 text-right text-muted-foreground tabular-nums">n={fmtCount(stats.count)}</div>
                </div>
              {/each}
              {#if p.scoreThreshold}
        ...
        {#if archetypeRows.length > 0}
          <Card.Root>
            <Card.Header class="pb-2">
              ...
              <Card.Description class="text-xs">
                Conversion rate per archetype (positive ÷ total). Apply more in green rows, less in red rows.
                "Unknown" rows are JDs we couldn't classify — usually a sign the report's Block A wasn't filled in.
              </Card.Description>
            </Card.Header>
            <Card.Content>
                      ...
                      <Tooltip.Trigger>
                        {#snippet child({ props })}
                          <div {...props} class="flex items-center gap-3 text-xs cursor-help">
                            <div class="w-32 shrink-0 truncate" title={row.archetype}>{row.archetype}</div>
                            <div class="flex-1 relative h-5 rounded bg-muted/30 overflow-hidden">
                              <div class={cn('absolute inset-y-0 left-0', rateTint(row.conversionRate))} style={'width: ' + rateWidth(row.conversionRate) + '%'}></div>
                            </div>
                            <div class="w-14 text-right tabular-nums">{fmtRate(row.conversionRate)}</div>
                            <div class="w-12 text-right text-muted-foreground tabular-nums">n={fmtCount(row.total)}</div>
                          </div>
                        {/snippet}
                      </Tooltip.Trigger>
          ...
          {#if remoteRows.length > 0}
            <Card.Root>
              <Card.Header class="pb-2">
                ...
                <Card.Description class="text-xs">
                  Conversion rate per work-mode. If "hybrid/onsite" sits at 0% across many apps, that's a
                  filter the system should be applying for you in <code class="font-mono text-foreground/80">portals.yml</code>.
                </Card.Description>
              </Card.Header>
              <Card.Content class="space-y-1">
                {#each remoteRows as row (row.policy)}
                  <div class="flex items-center gap-3 text-xs">
                    <div class="w-28 shrink-0 truncate capitalize" title={row.policy}>{row.policy}</div>
                    ...
                    <div class="w-10 text-right text-muted-foreground tabular-nums">n={fmtCount(row.total)}</div>
                  </div>
                {/each}
              </Card.Content>
            </Card.Root>
          ...
          {#if companySizeRows.length > 0}
            <Card.Root>
              <Card.Header class="pb-2">
                ...
                <Card.Description class="text-xs">
                  Conversion split by company stage / headcount band. Big differences here argue for narrowing
                  the targeting in your archetype, not just the keywords.
                </Card.Description>
              </Card.Header>
              <Card.Content class="space-y-1">
          ...
          {#if p.blockerAnalysis && p.blockerAnalysis.length > 0}
            <Card.Root>
              <Card.Header class="pb-2">
                ...
                <Card.Description class="text-xs">
                  Reasons cited in the gaps section of your evaluations. High-frequency blockers are the
                  filters worth tightening upstream so they don't burn evaluation tokens.
                </Card.Description>
              </Card.Header>
              <Card.Content class="space-y-1.5">
          ...
          {#if p.techStackGaps && p.techStackGaps.length > 0}
            <Card.Root>
              <Card.Header class="pb-2">
                ...
                <Card.Description class="text-xs">
                  Technologies named in JDs but not on your CV. Recurring gaps are candidates for either a
                  CV update or a filter that excludes JDs that lead with them.
                </Card.Description>
              </Card.Header>
              <Card.Content class="space-y-1.5">
                {#each p.techStackGaps.slice(0, 8) as g (g.skill)}
                  <div class="flex items-center gap-3 text-xs">
                    ...
                    <div class="font-mono tabular-nums text-muted-foreground">{fmtCount(g.frequency)}×</div>
                  </div>
                {/each}
              </Card.Content>
            </Card.Root>
        ...
        <p class="text-[10px] text-muted-foreground/60 text-center">
          Generated {p.metadata?.analysisDate ?? 'just now'} · cached for 10 minutes ·
          click Refresh to recompute against the latest tracker.
        </p>
      {/if}
    </div>
  </div>
