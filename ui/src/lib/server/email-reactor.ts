...
const STAGE_PATTERNS: Array<[RegExp, InterviewStage]> = [
  ...
  [
    /\bfinal (?:round|interview)\b|\bhiring (?:committee|manager) (?:final|interview)\b|\bvp interview\b/i,
    'Final',
  ],
  [/\btake[- ]home\b|\bcoding (?:test|challenge|exercise)\b/i, 'TakeHome'],
  [
    /\btechnical (?:interview|screen|round)\b|\bcoding (?:interview|round)\b|\bsystem design\b/i,
    'Technical',
  ],
  [
    /\bphone (?:screen|interview)\b|\brecruiter (?:call|screen)\b|\bhr screen\b|\bintro (?:call|chat)\b/i,
    'PhoneScreen',
  ],
];
...
export function classifyEmail(email: EmailInput): Classification {
  ...
  if (rejectionEvidence) {
    return {
      ...
      confidence: /after careful consideration|regret to inform/i.test(rejectionEvidence)
        ? 'high'
        : 'medium',
      senderDomain,
      ...
      switch (a.type) {
        ...
        case 'fire-takehome-scaffold':
          ...
          logEvent('email-reactor', 'Take-home scaffold fired', {
            ...
            category: 'application',
            message:
              "Working dir + README + checklist + state.json created · open the job's tech-prep menu",
            profileId: a.profileId,
          });
          result.executed++;
          break;
...
function fireBackgroundTechPrep(jobId: string, profileId?: string): void {
  ...
  void fetch('http://127.0.0.1:5174' + url, {
    ...
    logEvent('email-reactor', 'Background tech-prep failed', {
      level: 'warn',
      category: 'application',
...
function fireBackgroundPostRejection(jobId: string, profileId?: string): void {
  ...
  void fetch('http://127.0.0.1:5174' + url, {
    ...
    logEvent('email-reactor', 'Background post-rejection failed', {
      level: 'warn',
      category: 'application',
      message: err instanceof Error ? err.message : String(err),
    }),
...
function fireBackgroundTakehomeScaffold(jobId: string, profileId?: string): void {
  ...
  void fetch('http://127.0.0.1:5174' + url, {
    ...
    logEvent('email-reactor', 'Background takehome-scaffold failed', {
      level: 'warn',
      category: 'application',
      message: err instanceof Error ? err.message : String(err),
    }),
...
export function reactToEmail(email: EmailInput): {
  ...
  const match =
    classification.kind !== 'recruiter-reach-out' && classification.kind !== 'other'
