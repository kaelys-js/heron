/**
 * Province / state lists for the countries where the input is structured.
 * Other countries fall back to free-text input.
 *
 * For each country we include a "name" (display, the canonical full string
 * users expect on a CV like "British Columbia") and a short "code" for
 * places where abbreviations make sense.
 */

export type State = { code: string; name: string };

/** US 50 states + DC + 5 inhabited territories. */
