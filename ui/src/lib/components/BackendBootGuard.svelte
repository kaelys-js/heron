...
{:else}
  <div
    class="fixed inset-0 z-50 flex flex-col items-center justify-center gap-4 bg-background p-6 text-center"
  >
    <h1 class="text-xl font-semibold">Can't find your backend</h1>
    <p class="max-w-md text-sm text-muted-foreground">
      None of the discovery sources (embedded, localhost:5173, LAN, Tailscale, remote) answered.
      Open <code>/settings/backend</code> on a web browser to configure a remote URL, or run
      <code>pnpm dev</code> on a Mac on the same wifi.
    </p>
