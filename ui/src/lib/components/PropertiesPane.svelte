...
<aside class="w-72 flex-shrink-0 border-l bg-muted/10 overflow-y-auto">
  <div class="p-4 space-y-4">
    <Tooltip.Provider delayDuration={350}>
      ...
      <!-- ===== Section: Status & fit ===== -->
      <div class="space-y-2">
        <h3 class="text-[10px] font-semibold text-muted-foreground/80 uppercase tracking-wider flex items-center gap-1.5">
        ...
        <Tooltip.Root>
          ...
          <Tooltip.Content side="left" class="text-xs max-w-xs">{PROP_TIPS.Status}</Tooltip.Content>
        </Tooltip.Root>
        <dd>
          <DropdownMenu.Root>
              ...
              {#snippet child({ props })}
                ...
                >
                  <span class="flex items-center gap-1.5 min-w-0">
                    <span class={cn('size-1.5 rounded-full flex-shrink-0', STATUS_DOTS[job.status as Status])}></span>
        ...
        <dd>
          <span class={cn('text-xs font-mono font-semibold border rounded px-2 py-0.5 inline-block', scoreClass)}>
            {job.score != null
              ? job.score.toFixed(1)
              : job.geminiScore != null ? '~' + job.geminiScore.toFixed(0) : '—'}
          </span>
        ...
        <Tooltip.Root>
          ...
          <Tooltip.Content side="left" class="text-xs max-w-xs">{PROP_TIPS['BG risk']}</Tooltip.Content>
        </Tooltip.Root>
        <dd>
          {#if job.bgRisk}
            <Badge variant="outline" class={cn('text-[10px] uppercase font-mono h-5', BG_TINTS[job.bgRisk])}>{job.bgRisk}</Badge>
          {:else}
        ...
        <Tooltip.Root>
          ...
          <Tooltip.Content side="left" class="text-xs max-w-xs">{PROP_TIPS['Work mode']}</Tooltip.Content>
        </Tooltip.Root>
        <dd>
      ...
      <div class="space-y-2 pt-3 border-t border-border/30">
        ...
        <Tooltip.Root>
          ...
          <Tooltip.Content side="left" class="text-xs max-w-xs">{PROP_TIPS.Company}</Tooltip.Content>
        </Tooltip.Root>
        ...
        <Tooltip.Root>
          ...
          <Tooltip.Content side="left" class="text-xs max-w-xs">{PROP_TIPS.Location}</Tooltip.Content>
        </Tooltip.Root>
        ...
        <Tooltip.Root>
          ...
          <Tooltip.Content side="left" class="text-xs max-w-xs">{PROP_TIPS.Source}</Tooltip.Content>
        </Tooltip.Root>
        <dd class="min-w-0">
          {#if job.url}
            <Tooltip.Root>
          ...
          <dl class="grid grid-cols-[88px_1fr] gap-y-2.5 gap-x-3 text-xs items-center">
            {#if job.reportFile}
              <Tooltip.Root>
                ...
                <Tooltip.Content side="left" class="text-xs max-w-xs">{PROP_TIPS.Report}</Tooltip.Content>
              </Tooltip.Root>
              <dd class="truncate font-mono text-[10px] text-muted-foreground" title={job.reportFile}>
                {job.reportFile}
              </dd>
            {/if}
            ...
            {#if job.pdfFile}
              <Tooltip.Root>
                ...
                <Tooltip.Content side="left" class="text-xs max-w-xs">{PROP_TIPS['CV PDF']}</Tooltip.Content>
              </Tooltip.Root>
    ...
    {#if job.notes}
      <div class="pt-3 border-t border-border/30">
        <h4 class="text-[10px] font-semibold text-muted-foreground/80 uppercase tracking-wider mb-1.5">Notes</h4>
