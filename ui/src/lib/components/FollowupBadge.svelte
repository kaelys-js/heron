...
<script lang="ts">
  ...
  const LABEL: Record<Urgency, (e: FollowupEntry) => string> = {
    ...
    overdue: (e) => 'Late · ' + (e.daysUntilNext != null && e.daysUntilNext < 0 ? Math.abs(e.daysUntilNext) + 'd' : ''),
  ...
  const HINT: Record<Urgency, (e: FollowupEntry) => string> = {
    urgent: (e) =>
      'Applied ' + e.daysSinceApplication + ' days ago · ' +
      (e.followupCount === 0 ? 'no follow-up sent yet' : e.followupCount + ' follow-up' + (e.followupCount === 1 ? '' : 's')) +
      '. Today is the recommended day to nudge.',
    overdue: (e) =>
      'Last touched ' + (e.daysSinceLastFollowup ?? e.daysSinceApplication) + ' days ago · ' +
      'recommended next follow-up was ' + (e.daysUntilNext != null ? Math.abs(e.daysUntilNext) + 'd ago.' : 'overdue.'),
    waiting: (e) =>
      'Next follow-up suggested in ' + (e.daysUntilNext ?? '?') + ' days · stay patient until then.',
    cold: () =>
