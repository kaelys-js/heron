  ...
  <Card.Content class="space-y-4">
    ...
    <div class="flex items-center gap-3 text-xs flex-wrap">
      ...
      {#if stats.lastUpdatedAt}
        <span class="text-muted-foreground">·</span>
        <span class="text-muted-foreground"
          >last edit {formatRelativeTime(stats.lastUpdatedAt)}</span
        >
      {/if}
    ...
    <div class="rounded-md border border-border/40 bg-card px-3 py-3 space-y-2">
      ...
      <div class="flex items-center gap-2 flex-wrap">
        <Button
          size="sm"
          onclick={saveNew}
          disabled={saving || !newLabel.trim() || !newAnswer.trim()}
          class="gap-1.5"
        >
    ...
    {#if answers.length === 0}
      <div class="rounded-md border border-dashed border-border/40 px-3 py-6 text-center">
        ...
        <p class="text-xs text-muted-foreground">
          No answers saved yet. Seed common ones above, or they'll auto-save the first time you use
          the "Pre-fill application answers" action on a job.
        </p>
      </div>
    {:else}
      <div class="space-y-1.5">
        {#each answers as row (row.key)}
          {#if editingKey === row.key}
            <div
              class="rounded-md border border-fuchsia-500/40 bg-fuchsia-500/5 px-3 py-2.5 space-y-2"
            >
              ...
              <div class="flex items-center gap-2">
                <Button size="sm" onclick={saveEdit} disabled={saving} class="gap-1.5">
                  {#if saving}<Loader2 class="size-3 animate-spin" />{:else}<Save
                      class="size-3"
                    />{/if}
                  Save
                </Button>
          ...
          {:else}
            <div
              class="flex items-start gap-3 rounded-md border border-border/40 bg-card px-3 py-2"
            >
              <div class="flex-1 min-w-0 space-y-0.5">
                <div class="text-xs font-medium truncate">{row.label}</div>
                <div
                  class="text-[11px] text-muted-foreground/80 whitespace-pre-wrap line-clamp-2 leading-snug"
                >
                  {row.answer}
                </div>
