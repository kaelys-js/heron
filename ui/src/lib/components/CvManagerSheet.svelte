  ...
  <Sheet.Content side="right" class="w-full sm:max-w-2xl flex flex-col p-0 gap-0">
    <Sheet.Header class="px-5 pt-5 pb-3 border-b">
      <div class="flex items-start gap-3">
        <div class="size-10 rounded-lg bg-foreground/5 ring-1 ring-border flex items-center justify-center flex-shrink-0">
        ...
        <div class="flex-1 min-w-0">
          ...
          <Sheet.Description class="text-xs mt-0.5">
            <code class="font-mono text-foreground/80">cv.md</code> is the canonical CV — every PDF, score, and cover letter reads from it.
          </Sheet.Description>
        </div>
      </div>
    </Sheet.Header>

    <Tabs.Root value={activeTab} onValueChange={(v: string) => (activeTab = v as Tab)} class="flex-1 flex flex-col min-h-0">
          ...
          <div class="px-5">
            <div class="flex items-start gap-2 px-3 py-2.5 rounded-md border border-amber-500/40 bg-amber-500/10 text-xs">
              ...
              <div class="text-amber-200/90 flex-1">
                <strong>cv.md not found.</strong> Use the Replace tab to paste a CV — we'll create the file for you.
              </div>
            </div>
          </div>
        {:else}
            ...
            <div class="flex items-center gap-1">
              <Tooltip.Provider delayDuration={300}>
                <Tooltip.Root>
                  <Tooltip.Trigger>
                    {#snippet child({ props })}
                      <Button {...props} variant="ghost" size="sm" class="h-7 gap-1 text-xs" onclick={copyBody}>
                        {#if copyState === 'copied'}
                  ...
                  <Tooltip.Content side="bottom" class="text-xs">Copy raw markdown to clipboard</Tooltip.Content>
                </Tooltip.Root>
                <Tooltip.Root>
                  <Tooltip.Trigger>
                    {#snippet child({ props })}
                      <Button {...props} variant="ghost" size="sm" class="h-7 gap-1 text-xs" onclick={downloadBody}>
                        <Download class="size-3" /> Download
                      </Button>
                    {/snippet}
                  </Tooltip.Trigger>
                  <Tooltip.Content side="bottom" class="text-xs">Save cv.md to your Downloads folder</Tooltip.Content>
                </Tooltip.Root>
              </Tooltip.Provider>
            </div>
          </div>
          <div class="flex-1 overflow-y-auto px-5 pb-4">
            <article class="prose prose-invert prose-sm max-w-none prose-headings:font-semibold prose-headings:tracking-tight prose-pre:bg-muted prose-strong:text-foreground">
              {@html bodyHtml}
            </article>
          </div>
        {/if}
        ...
        {:else}
          <div class="flex items-center justify-between gap-2 px-5 pb-2">
            <div class="text-[10px] text-muted-foreground">
              Edit markdown directly. Changes are saved to <code class="font-mono text-foreground/80">cv.md</code> with a backup to <code class="font-mono text-foreground/80">cv.md.bak</code>.
            </div>
            {#if editDirty}
          ...
          <div class="px-5 py-3 border-t bg-muted/20 flex items-center justify-end gap-2">
            <Button
              ...
              class={cn(
                ...
                editDiscardArmed && 'bg-red-500/15 text-red-300 hover:bg-red-500/25 ring-1 ring-red-500/40 animate-pulse',
              )}
            >
              <RotateCcw class="size-3.5" /> {editDiscardArmed ? 'Click again to discard' : 'Discard'}
            </Button>
        ...
        <div class="px-5 pb-3 space-y-2">
          <div class="flex items-start gap-2 px-3 py-2.5 rounded-md border border-amber-500/40 bg-amber-500/10 text-xs">
            ...
            <div class="text-amber-200/90 leading-relaxed">
              ...
              <code class="font-mono">cv.md.bak</code> so you can recover if needed. After replacing, run
          ...
          <p class="text-[11px] text-muted-foreground/80 leading-relaxed">
            Paste your new CV in markdown format. Plain text works too — just won't render with headings/lists in the View tab.
          </p>
        </div>
        <div class="flex-1 overflow-hidden px-5">
          <Textarea
        ...
        <div class="px-5 py-3 border-t bg-muted/20 flex items-center justify-between gap-2">
          ...
          <div class="flex items-center gap-2">
            ...
            <Button
              ...
              class={cn(
                'gap-1.5 transition-all',
                replaceArmed && 'bg-red-500/20 text-red-300 hover:bg-red-500/30 border border-red-500/40 animate-pulse',
              )}
            >
              {#if replaceBusy}
      ...
      <Tabs.Content value="reprocess" class="flex-1 min-h-0 flex flex-col mt-3">
        <div class="px-5 pb-3 space-y-3">
          <div class="rounded-md border border-border/40 bg-muted/30 p-3 space-y-2">
            ...
            <ul class="text-[11px] text-muted-foreground/80 leading-relaxed space-y-0.5 list-disc list-inside ml-1">
              <li>Reads <code class="font-mono">cv.md</code> and asks Claude to extract structured fields.</li>
              <li>Returns a <em>suggestion</em> for: name · email · phone · LinkedIn · GitHub · portfolio · headline · exit story · superpowers · proof points · location.</li>
              <li>Nothing is saved until you click <strong>Apply suggestions</strong>, then <strong>Save Profile</strong> on the main page.</li>
            </ul>
          </div>
          <div class="flex items-start gap-2 px-3 py-2 rounded-md border border-border/40 bg-card text-[11px] text-muted-foreground">
            <Info class="size-3.5 text-blue-400 mt-0.5 flex-shrink-0" />
            <p class="leading-relaxed">
              Costs one Anthropic API call (~$0.10–$0.30 on Opus depending on CV size). Requires <code class="font-mono">ANTHROPIC_API_KEY</code> in Settings.
            </p>
          </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto px-5 pb-4 space-y-3">
          {#if reprocessError && !suggestion}
            <div class="flex items-start gap-2 px-3 py-2.5 rounded-md border border-red-500/40 bg-red-500/10 text-xs">
          ...
          {#if suggestion}
            <div class="text-[10px] uppercase tracking-wider text-muted-foreground/70 font-medium">
              Proposed updates ({suggestionSummary.length} field{suggestionSummary.length === 1 ? '' : 's'})
            </div>
            {#if suggestionSummary.length === 0}
              <div class="text-xs text-muted-foreground/80 italic">
                Claude couldn't derive any usable fields from this CV. Make sure cv.md has the candidate's contact info, role headline, and experience.
              </div>
            {:else}
              <ul class="space-y-1.5">
                {#each suggestionSummary as s}
                  <li class="flex items-baseline gap-3 text-xs px-3 py-2 rounded-md border border-border/40 bg-card">
                    <span class="w-24 text-muted-foreground flex-shrink-0">{s.label}</span>
                    <span class="text-foreground flex-1 min-w-0 break-words leading-relaxed">{s.sample}</span>
                  </li>
                {/each}
              </ul>
            {/if}
          ...
          <div class="flex items-center gap-2">
            {#if suggestion}
              <Button
                ...
                class={cn(
                  'gap-1.5 transition-all',
                  reprocessDiscardArmed && 'bg-red-500/15 text-red-300 hover:bg-red-500/25 ring-1 ring-red-500/40 animate-pulse',
                )}
              >
                <RotateCcw class="size-3.5" /> {reprocessDiscardArmed ? 'Click again to discard' : 'Discard'}
              </Button>
              <Button
                size="sm"
