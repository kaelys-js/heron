...
<section class="rounded-md border border-border/40 bg-card overflow-hidden">
  ...
  >
    <ChevronDown class={cn('size-3.5 text-muted-foreground transition-transform flex-shrink-0', !open && '-rotate-90')} />
    ...
    <div class="flex items-center gap-3 text-[10px] text-muted-foreground/80 flex-shrink-0">
      <span class="tabular-nums">
        <span class="text-foreground font-medium">{jobs.length}</span> {jobs.length === 1 ? 'role' : 'roles'}
      </span>
      {#if stats.topScore != null}
        <span class="inline-flex items-center gap-1">
          <span class="text-muted-foreground/60">top</span>
          <span class={cn('font-mono tabular-nums', topScoreClass)}>{stats.topScore.toFixed(1)}</span>
        </span>
      {/if}
      {#if stats.ready > 0}
  ...
  >
    <div class={cn('overflow-hidden min-h-0', !open && 'pointer-events-none')}>
      <div class="border-t border-border/40">
        ...
        <div class="flex items-center gap-3 px-3 h-7 text-[10px] uppercase tracking-wider text-muted-foreground/70 border-b border-border/30 bg-muted/20 border-l-2 border-l-transparent">
