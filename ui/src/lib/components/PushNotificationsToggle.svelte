...
<script lang="ts">
  ...
  async function requestPermission() {
    if (permission === 'unsupported') {
      toast.error('Notifications not supported', { description: 'Your browser doesn\'t expose the Notification API.' });
      return;
    }
    if (permission === 'denied') {
      toast.error('Notifications blocked', { description: 'Re-enable in your browser settings (Site permissions).' });
      return;
    }
    try {
      ...
      if (result === 'granted') {
        toast.success('Notifications enabled', { description: 'You\'ll get OS-level pings for important events.' });
      } else {
        toast.info('Permission not granted', { description: 'You can enable later from your browser settings.' });
      }
    } catch (e) {
