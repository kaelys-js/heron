...
{#if total > pageSize}
  <div class={cn('flex items-center justify-between gap-2 px-3 py-2 text-[11px] text-muted-foreground border-t border-border/40', className)}>
    <span class="tabular-nums">
      {#if allShown}
        Showing all {total.toLocaleString()} {label}
      {:else}
        Showing <span class="text-foreground font-medium">{visible.toLocaleString()}</span> of {total.toLocaleString()} {label}
