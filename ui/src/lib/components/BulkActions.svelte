      ...
      <Tooltip.Provider delayDuration={300}>
        <Tooltip.Root>
          ...
          <Tooltip.Content side="bottom" class="text-xs max-w-xs">
            ...
            {applyLinkedInCount} via LinkedIn auto + {applyOtherCount} marked Applied with browser tabs opened.
          </Tooltip.Content>
        </Tooltip.Root>
      </Tooltip.Provider>
    {/if}
    ...
    {#if cvWillRun > 0}
      <Tooltip.Provider delayDuration={300}>
        <Tooltip.Root>
          ...
          <Tooltip.Content side="bottom" class="text-xs max-w-xs">
            Run Claude oferta on all {cvWillRun} job{cvWillRun === 1 ? '' : 's'} (sequentially, ~1–3 min each).
          </Tooltip.Content>
        </Tooltip.Root>
      </Tooltip.Provider>
    {/if}
...
<Dialog.Root bind:open={applyOpen}>
  <Dialog.Content class="sm:max-w-lg p-0 gap-0 overflow-hidden">
    ...
    <div class="px-5 py-4 space-y-3">
      ...
      >
        <Linkedin class={cn('size-4 mt-0.5 flex-shrink-0', applyLinkedInCount > 0 ? 'text-blue-400' : 'text-muted-foreground/60')} />
        <div class="flex-1 min-w-0 space-y-1">
          ...
          <p class="text-[11px] text-muted-foreground/80 leading-relaxed">
            Playwright fills these in headed mode — you can watch and click Submit yourself.
            They run sequentially (rate limits + safety).
          </p>
        </div>
      </div>
      ...
      >
        <ArrowUpRight class={cn('size-4 mt-0.5 flex-shrink-0', applyOtherCount > 0 ? 'text-violet-400' : 'text-muted-foreground/60')} />
        <div class="flex-1 min-w-0 space-y-1">
          <div class="flex items-baseline gap-2">
          ...
          <p class="text-[11px] text-muted-foreground/80 leading-relaxed">
            We open each posting in a new tab (300ms stagger so your browser doesn't block) and
            flip the tracker to Applied. You finish the form on each company's site.
          </p>
        </div>
      </div>
      ...
      <div class="rounded-md border border-border/40 bg-muted/30 p-3 flex items-start gap-2">
        ...
        <div class="flex-1 min-w-0 space-y-1">
          ...
          <ul class="text-[10px] text-muted-foreground/80 leading-relaxed space-y-0.5 list-disc list-inside">
            <li>Activity feed (bell, top-right) gets a "Bulk apply ↦" event per job.</li>
            <li>Toast pops with the per-step outcome.</li>
            <li>If anything fails, the failure toast shows a <span class="font-medium text-foreground">Retry</span> button.</li>
      ...
      {#if applyOtherCount > 5}
        <div class="rounded-md border border-amber-500/30 bg-amber-500/5 p-3 flex items-start gap-2">
          <AlertTriangle class="size-3.5 text-amber-400 mt-0.5 flex-shrink-0" />
          <p class="text-[10px] text-amber-200/90 leading-relaxed">
            Browsers may block more than ~5 simultaneous tab opens. If you see fewer tabs than expected,
            allow pop-ups for this site and re-run for the missing rows.
          </p>
        </div>
      {/if}
    </div>
    ...
    <Dialog.Footer class="px-5 py-3 border-t bg-muted/20">
      <Button variant="ghost" onclick={() => (applyOpen = false)} disabled={applyBusy}>Cancel</Button>
      <Button onclick={submitApply} disabled={applyBusy || applyTotal === 0} class="gap-1.5">
        ...
        {:else}
          <Send class="size-3.5" />
          Apply to {applyTotal} {applyTotal === 1 ? 'job' : 'jobs'}
        {/if}
      </Button>
    </Dialog.Footer>
  </Dialog.Content>
  ...
  <Dialog.Content class="sm:max-w-lg p-0 gap-0 overflow-hidden">
    <Dialog.Header class="px-5 pt-5 pb-3 border-b">
      <div class="flex items-start gap-3">
        ...
        <div class="flex-1 min-w-0">
          <Dialog.Title class="text-base">Generate {cvWillRun} tailored CV{cvWillRun === 1 ? '' : 's'}</Dialog.Title>
          <Dialog.Description class="text-xs mt-0.5">
    ...
    <div class="px-5 py-4 space-y-3">
      <div class="rounded-md border border-border/40 bg-muted/20 p-3 space-y-2">
        ...
        <p class="text-[10px] text-muted-foreground/70 pt-1">
          Roughly 1–3 minutes per job. {cvWillRun} jobs → estimated {Math.max(1, Math.ceil(cvWillRun * 1.5))}–{cvWillRun * 3} minutes.
        </p>
      </div>
      ...
      <div class="rounded-md border border-border/40 bg-muted/30 p-3 flex items-start gap-2">
        ...
        <div class="flex-1 min-w-0 space-y-1">
          ...
          <ul class="text-[10px] text-muted-foreground/80 leading-relaxed space-y-0.5 list-disc list-inside">
            ...
            <li>Each report and PDF appears in <span class="font-mono">reports/</span> + <span class="font-mono">output/</span>.</li>
      ...
      <div class="rounded-md border border-border/40 bg-muted/20 p-3 flex items-start gap-2">
        ...
        <p class="text-[10px] text-muted-foreground/80 leading-relaxed">
          ...
          If you use a different CLI (Gemini / Codex / OpenCode), the orchestrator may not pick it up — pin Claude Code
          for this feature.
        </p>
      </div>
    </div>
