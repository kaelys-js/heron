...
>
  <a href={'/job/' + job.id} class="flex items-center gap-3 flex-1 min-w-0">
    <Tooltip.Provider delayDuration={400}>
      <!-- 1. Status dot (with tooltip explaining the state) -->
      <Tooltip.Root>
        <Tooltip.Trigger>
          {#snippet child({ props })}
            <span {...props} class={cn('size-1.5 rounded-full flex-shrink-0', statusDotClass)}
            ></span>
          {/snippet}
        </Tooltip.Trigger>
        ...
        <Tooltip.Trigger>
          {#snippet child({ props })}
            <span
              {...props}
              class={cn(
                'font-mono tabular-nums w-8 text-right flex-shrink-0 cursor-help',
                scoreClass,
              )}
            >
      ...
      <Tooltip.Root>
        <Tooltip.Trigger>
          {#snippet child({ props })}
            <span {...props} class="font-medium truncate flex-1 min-w-0 cursor-help"
              >{job.role}</span
            >
          {/snippet}
        </Tooltip.Trigger>
      ...
      <span
        class="text-muted-foreground/80 inline-flex items-center gap-1 truncate w-36 flex-shrink-0"
      >
        {#if job.location}
            ...
            <Tooltip.Trigger>
              {#snippet child({ props })}
                <span
                  {...props}
                  class={cn(
                    'text-[9px] uppercase font-mono px-1 py-0 rounded border h-4 inline-flex items-center cursor-help',
                    BG_TINTS[bg],
                  )}
                >
                  {bg}
