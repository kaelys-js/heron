    ...
    <div class="ml-auto flex items-center gap-0.5">
      {#if jobs.length > PAGE_SIZE}
        <DropdownMenu.Root>
                ...
                {#snippet child({ props: tipProps })}
                  <DropdownMenu.Trigger>
                    {#snippet child({ props: ddProps })}
                      <Button {...tipProps} {...ddProps} variant="ghost" size="icon" class="h-6 w-6" aria-label="Column options">
          ...
          <DropdownMenu.Content side="bottom" align="end" class="w-52">
            <DropdownMenu.Label class="text-[10px] uppercase tracking-wide text-muted-foreground">{title} · {jobs.length}</DropdownMenu.Label>
      ...
      <Tooltip.Provider delayDuration={200}>
        <Tooltip.Root>
          <Tooltip.Trigger>
            {#snippet child({ props })}
              <Button {...props} variant="ghost" size="icon" class="h-6 w-6" onclick={() => (addOpen = true)}>
                <Plus class="size-3" />
              </Button>
            {/snippet}
          </Tooltip.Trigger>
  ...
  <Dialog.Content class="sm:max-w-md">
    ...
    <div class="space-y-3">
      <div class="space-y-1.5">
        <label for="add-url" class="text-xs font-medium">URL <span class="text-red-400">*</span></label>
        <Input id="add-url" type="url" bind:value={addUrl} placeholder="https://..." class="font-mono text-sm" />
      </div>
