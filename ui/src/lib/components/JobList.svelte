...
<div class="border rounded-lg overflow-hidden bg-muted/10">
  <Tooltip.Provider delayDuration={300}>
    ...
    <div class="grid grid-cols-[60px_minmax(220px,_3fr)_1fr_1fr_70px_70px_60px_minmax(140px,_1.2fr)_60px_100px] gap-3 px-3 py-2 text-[10px] uppercase tracking-wider text-muted-foreground border-b font-medium bg-muted/20">
      <Tooltip.Root>
        <Tooltip.Trigger>
          {#snippet child({ props })}<div {...props} class="text-right cursor-help">Score</div>{/snippet}
        </Tooltip.Trigger>
      ...
      <Tooltip.Root>
        <Tooltip.Trigger>
          {#snippet child({ props })}<div {...props} class="text-right cursor-help">Files</div>{/snippet}
        </Tooltip.Trigger>
        <Tooltip.Content side="bottom" class="text-xs">Has report / Has tailored CV PDF</Tooltip.Content>
      </Tooltip.Root>
      <Tooltip.Root>
        <Tooltip.Trigger>
          {#snippet child({ props })}<div {...props} class="text-right cursor-help">Actions</div>{/snippet}
        </Tooltip.Trigger>
    ...
    {#each jobs as job, idx (job.id)}
      ...
      >
        ...
        <div class="text-right">
          {#if score != null}
            <Tooltip.Root>
              <Tooltip.Trigger>
                {#snippet child({ props })}
                  <span {...props} class={cn('text-[10px] font-mono font-semibold border rounded px-1.5 py-0.5 cursor-help', scoreClass(score))}>
                    {score.toFixed(1)}
                  </span>
                {/snippet}
              </Tooltip.Trigger>
        ...
        <!-- Location -->
        <div class="truncate text-xs text-muted-foreground/80" title={job.location}>{job.location || '—'}</div>
        ...
        <div>
          <Tooltip.Root>
            <Tooltip.Trigger>
              {#snippet child({ props })}
                <Badge {...props} variant="outline" class={cn('text-[10px] uppercase font-mono border cursor-help', STATUS_TINTS[job.status])}>{job.status}</Badge>
              {/snippet}
            </Tooltip.Trigger>
            <Tooltip.Content side="top" class="text-xs max-w-xs">{STATUS_HINT[job.status]}</Tooltip.Content>
          </Tooltip.Root>
        </div>
        ...
        <div>
          <Tooltip.Root>
            <Tooltip.Trigger>
              {#snippet child({ props })}
                <span {...props} class={cn('inline-flex items-center gap-1 text-[10px] cursor-help', wm.tint)}>
                  <WIcon class="size-3" />
        ...
        <div>
          {#if job.bgRisk}
            ...
            <Tooltip.Root>
              <Tooltip.Trigger>
                {#snippet child({ props })}
                  <Badge {...props} variant="outline" class={cn('text-[10px] uppercase font-mono border cursor-help', BG_TINTS[bg])}>{bg}</Badge>
                {/snippet}
              </Tooltip.Trigger>
              <Tooltip.Content side="top" class="text-xs max-w-xs">
        ...
        <div class="min-w-0">
          {#if job.salary}
            <Tooltip.Root>
              <Tooltip.Trigger>
                {#snippet child({ props })}
                  <span {...props} class="inline-flex items-center gap-0.5 text-[11px] text-emerald-400/85 cursor-help truncate max-w-full">
