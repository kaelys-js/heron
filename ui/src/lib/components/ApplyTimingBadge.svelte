...
<script lang="ts">
  ...
  let { jobId, profileId, status }: { jobId: string; profileId?: string; status: Status } =
    $props();
  ...
  <Popover.Root>
    ...
    <Popover.Content class="w-80 p-3 space-y-2">
      ...
      <div class="text-[11px] text-muted-foreground space-y-0.5">
        ...
        {#if timing.daysSinceFirstSeen != null}
          <p>
            Today is Day <span class="font-mono text-foreground"
              >{timing.daysSinceFirstSeen + 1}</span
