    ...
    <div class="flex-1 flex flex-col items-center gap-1 min-w-0">
      <div class="w-full h-full flex flex-col-reverse items-stretch">
        <div class="relative w-full rounded-t" style={'height: ' + Math.max(2, totalH) + 'px'}>
          <div
            class={'absolute inset-x-0 bottom-0 rounded-t ' + (b.tint ?? 'bg-muted')}
            style={'height: ' + Math.max(2, totalH) + 'px'}
          ></div>
          {#if b.sub}
            <div
              class={'absolute inset-x-0 bottom-0 rounded-t ' + (b.sub.tint ?? 'bg-primary')}
              style={'height: ' + Math.max(0, subH) + 'px'}
            ></div>
          {/if}
        </div>
      </div>
