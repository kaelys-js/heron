<script lang="ts">
  ...
  const VIEWS: { key: ViewMode; label: string; icon: any; desc: string }[] = [
    ...
    { key: 'table', label: 'Table', icon: Table2, desc: 'Sortable spreadsheet' },
    {
      ...
      label: 'By company',
      icon: Building2,
      desc: 'Grouped sections per company',
    },
  ];
  ...
  const STATUS_HINT: Record<Status, string> = {
    ...
    Ghosted: 'No response for ≥21 days — auto-flagged',
  ...
  function resetSection(section: 'score' | 'bg' | 'workMode' | 'extras' | 'source') {
    ...
    else if (section === 'workMode')
      filter = { ...filter, workMode: { ...DEFAULT_FILTER.workMode } };
    ...
    <div class="flex items-center gap-2 text-sm flex-1 min-w-0">
      {#if crumb}
        <a
          ...
          data-sveltekit-preload-data="hover">{crumb}</a
        >
      ...
      <div class="flex items-center gap-1">
        ...
        <Popover.Root>
          ...
          <Popover.Content
            side="bottom"
            align="end"
            class="w-[400px] p-0 max-h-[80vh] overflow-y-auto"
          >
            <!-- Header -->
            <div
              class="flex items-center justify-between px-4 py-3 border-b sticky top-0 bg-popover z-10"
            >
            ...
            <div class="px-4 py-4 space-y-5">
              ...
              <section class="space-y-2 pt-1 border-t border-border/40">
                ...
                <div class="space-y-1.5">
                  <div
                    class="flex items-center justify-between gap-2 px-2.5 py-2 rounded-md hover:bg-muted/40 transition-colors"
                  >
                    <Label
                      for="f-report"
                      class="text-xs cursor-pointer flex items-center gap-2 flex-1"
                    >
                    ...
                    <Switch
                      id="f-report"
                      checked={filter.hasReport}
                      onCheckedChange={(v: boolean) => (filter = { ...filter, hasReport: !!v })}
                    />
                  </div>
                  <div
                    class="flex items-center justify-between gap-2 px-2.5 py-2 rounded-md hover:bg-muted/40 transition-colors"
                  >
                    ...
                    <Switch
                      ...
                      onCheckedChange={(v: boolean) => (filter = { ...filter, hasPdf: !!v })}
                    />
                  </div>
                  <div
                    class="flex items-center justify-between gap-2 px-2.5 py-2 rounded-md hover:bg-muted/40 transition-colors"
                  >
                    ...
                    <Switch
                      id="f-salary"
                      checked={filter.hasSalary}
                      onCheckedChange={(v: boolean) => (filter = { ...filter, hasSalary: !!v })}
                    />
                  </div>
                </div>
              </section>
      ...
      <Popover.Root>
        <Popover.Trigger>
          {#snippet child({ props })}
            <Button
              ...
              variant="ghost"
              size="sm"
              class={cn('h-8 gap-1.5 text-xs relative', activeFilterCount > 0 && 'text-foreground')}
            >
              <Filter class="size-3.5" />
              Filter
              {#if activeFilterCount > 0}
        ...
        >
          ...
          <div class="px-4 py-3 space-y-4 text-xs text-muted-foreground/80">
            <p>
              Use the Pipeline page's full filter UI for granular control. This minimal popover lets
              you set a minimum score and reset the filter from any page; the same FilterState is
            ...
            <div class="space-y-1.5">
              ...
              <input
                ...
                oninput={(e) =>
                  (filter = {
                    ...filter,
                    minScore: parseFloat((e.currentTarget as HTMLInputElement).value),
                  })}
                class="w-full"
              />
            ...
            <a
              href="/pipeline"
              class="block text-[11px] underline underline-offset-2 text-muted-foreground hover:text-foreground"
            >
