...
<script lang="ts">
  ...
  import { resolveBackend, pillLabel, type BackendSource, type ResolvedBackend } from '$lib/client/backend-discovery';
...
{#if resolved}
  <a
    ...
    class="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-[10px] font-mono uppercase tracking-wide transition-colors {colorBySource[resolved.source]}"
...
{:else if isCapacitor}
  <span class="inline-flex items-center gap-1.5 rounded-full border border-rose-500/40 bg-rose-500/20 px-2.5 py-0.5 text-[10px] font-mono uppercase tracking-wide text-rose-200">
    OFFLINE
  </span>
{/if}
