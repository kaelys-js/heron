...
<script lang="ts">
  ...
  const colorBySource: Record<BackendSource, string> = {
    embedded: 'bg-emerald-500/20 text-emerald-700 dark:text-emerald-200 border-emerald-500/40',
    dev: 'bg-violet-500/20 text-violet-700 dark:text-violet-200 border-violet-500/40',
    lan: 'bg-amber-500/20 text-amber-700 dark:text-amber-200 border-amber-500/40',
    tailscale: 'bg-sky-500/20 text-sky-700 dark:text-sky-200 border-sky-500/40',
    remote: 'bg-slate-500/20 text-slate-700 dark:text-slate-200 border-slate-500/40',
    manual: 'bg-rose-500/20 text-rose-700 dark:text-rose-200 border-rose-500/40',
  };
...
{:else if isCapacitor}
  <span
    class="inline-flex items-center gap-1.5 rounded-full border border-rose-500/40 bg-rose-500/20 px-2.5 py-0.5 text-[11px] font-mono uppercase tracking-wide text-rose-700 dark:text-rose-200"
  >
    OFFLINE
  </span>
{/if}
