    ...
    {:else}
        ...
        <div class="relative">
          {#if prefs.avatarPath}
            <img
              src={avatarSrc()}
              alt="avatar"
              class="size-16 rounded-full object-cover border border-border/40"
            />
          {:else}
            <div
      ...
      <div class="space-y-2 pt-2 border-t border-border/30">
        ...
        <div class="grid grid-cols-3 gap-1.5">
          {#each [{ id: 'system' as const, label: 'System', icon: Monitor }, { id: 'light' as const, label: 'Light', icon: Sun }, { id: 'dark' as const, label: 'Dark', icon: Moon }] as opt}
            <button
              type="button"
                ...
                onchange={(e) =>
                  patchPrefs({
                    notifications: {
                      ...prefs.notifications,
                      toast: {
                        ...prefs.notifications.toast,
                        [opt.level]: (e.currentTarget as HTMLInputElement).checked,
                      },
                    },
                  })}
