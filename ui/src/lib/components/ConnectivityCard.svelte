<!--
  ConnectivityCard — the shared full-screen shell for the two backend
  connectivity states (BackendBootGuard's "can't connect" gate + the
  BackendUnreachableOverlay live-disconnect blocker).

  It owns the chrome the two states have in common: a fixed full-screen
  container, the centralized BloomBackground, a theme-tokened glass card,
  and the reed-green brand-mark hero. The state-specific parts (eyebrow,
  body copy, action buttons, connection-detail disclosures) come in via
  named snippet slots so each caller keeps its own logic + a11y wiring.

  Layout order inside the card: brand mark → eyebrow? → heading → body? →
  actions → details?.

  The brand mark mirrors branding/logo.svg via apply-brand's
  AUTO-GENERATED:brand-mark marker pair (gradient-id="conn-grad"). Do NOT
  hand-edit between the markers; regenerate with `pnpm brand:apply`.
-->
<script lang="ts">
...
>
  ...
  >
    ...
    {#if eyebrow}
      <div class="text-[11px] font-semibold uppercase tracking-wider {eyebrowClass}">
        {@render eyebrow()}
      </div>
    {/if}

    <h2 id={headingId} class="text-2xl font-semibold tracking-tight text-foreground">
      {@render heading()}
    </h2>

    {#if body}
      <p class="text-sm leading-relaxed text-muted-foreground">{@render body()}</p>
    {/if}
