...
<script lang="ts">
  ...
  import BloomBackground from './BloomBackground.svelte';
  ...
  // Varied bar widths so skeleton rows don't repeat mechanically at rows≥4.
  const SKELETON_WIDTHS = [62, 95, 88, 74, 91, 81, 97];
</script>
...
{:else if variant === 'overlay'}
  <div
    class="fixed inset-0 z-50 flex flex-col items-center justify-center gap-3 overflow-hidden bg-background/90 backdrop-blur-sm"
  ...
  >
    ...
    >
      ...
      <div class="text-center">
        <p class="text-base font-semibold tracking-tight text-foreground">{message}</p>
        {#if sub}
...
{:else if variant === 'skeleton'}
  <div class="space-y-2.5" aria-busy="true">
    {#each Array(rows) as _, i}
      <div
        class="{i === 0 ? 'h-5' : 'h-4'} animate-pulse rounded bg-muted/60"
...
{:else}
  <div
    ...
    aria-live="polite"
  >
    <Loader2 class="{iconClass} animate-spin text-primary" />
    <div>
