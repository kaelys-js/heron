...
<div class={cn('inline-flex items-stretch h-9 rounded-md border bg-background overflow-hidden focus-within:ring-2 focus-within:ring-ring/40 transition-shadow', invalid && 'border-red-500/60 focus-within:ring-red-500/30', className)}>
  <button
    type="button"
    onclick={() => bump(-step)}
    disabled={value <= min}
  ...
  <div class="relative flex-1 flex items-center justify-center min-w-[3.5rem] px-1">
    ...
    />
    {#if suffix}
      <span class="absolute right-1.5 text-[10px] text-muted-foreground/70 pointer-events-none">{suffix}</span>
    {/if}
  </div>
