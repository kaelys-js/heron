...
function applyAndroidKotlinBrand(brand) {
  // Kotlin-side counterpart of Brand.swift / brand.ts. Brand.kt lives in
  // the Android code namespace (build.gradle `namespace = "..."`) -- the
  // package every authored Android source declares. The STORE identity
  // (applicationId / Brand.bundleId) derives from brand.json separately;
  // Android allows namespace != applicationId, so a bundleId rename never
  // forces a source-tree move. Read the namespace from build.gradle and
  // fall back to bundleId for a fresh scaffold that hasn't diverged.
  const gradlePath = join(UI, 'android', 'app', 'build.gradle');
  let namespace = brand.identifiers.bundleId;
  if (existsSync(gradlePath)) {
    const m = readFileSync(gradlePath, 'utf8').match(/namespace\s*=\s*"([^"]+)"/);
    if (m) namespace = m[1];
  }
  const pkgDir = namespace.split('.').join('/');
  const path = join(UI, 'android', 'app', 'src', 'main', 'java', pkgDir, 'Brand.kt');
  // Create the package dir if absent -- this used to silently skip ("run
  // cap sync android"), leaving a stale Brand.kt with the old brand.
  mkdirSync(dirname(path), { recursive: true });
  const body = [
...
function applyClientBrandTs(brand) {
  ...
  const body = [
    ...
    `  community: ${JSON.stringify(brand.community ?? {}, null, 2).replace(/\n/g, '\n  ')},`,
    `} as const;`,
    ``,
    `/** Community Discord invite URL. Empty string when not configured. */`,
    `export const DISCORD_URL: string = ${JSON.stringify(brand.community?.discord?.url ?? '')};`,
    ``,
...
function applySwiftConstants(brand) {
  ...
  const paths = [
    ...
    // the Watch had to hardcode a literal App Group id and
  ...
  const body = [
    ...
    `    /// "${brand.identifiers.bundleId}.handoff.interview-prep". iPhone's AppDelegate`,
...
function applyAGENTSMd(brand) {
  ...
  // The brand-derived strings inside it (com.resistjs.heron, heron://)
...
function applyWatchInfoPlist(brand) {
  // The watchOS target's Info.plist references its paired phone app via
  // WKCompanionAppBundleIdentifier. It MUST equal the iOS app's bundleId
  // or the watch app won't install -- derive it from brand.json.
  const path = join(UI, 'ios', 'App', 'WatchApp', 'Info.plist');
  if (!existsSync(path)) {
    log.skip(`WatchApp/Info.plist — missing`);
    return;
  }
  const body = readFileSync(path, 'utf8');
  const next = body.replace(
    /(<key>WKCompanionAppBundleIdentifier<\/key>\s*\n\s*<string>)[^<]*(<\/string>)/,
    `$1${brand.identifiers.bundleId}$2`,
  );
  if (next !== body) {
    writeFileSync(path, next);
    log.ok(`WatchApp/Info.plist`);
  } else {
    log.skip(`WatchApp/Info.plist — already current`);
  }
}

function applyMasEntitlements(brand) {
  // Mac App Store entitlements declare the App Group shared with a
  // sideloaded iOS app under Catalyst. Keep it in lock-step with
  // brand.identifiers.appGroup so a rebrand can't orphan the container.
  const path = join(UI, 'electron', 'build', 'entitlements.mas.plist');
  if (!existsSync(path)) {
    log.skip(`entitlements.mas.plist — missing`);
    return;
  }
  const body = readFileSync(path, 'utf8');
  const next = body.replace(
    /(<key>com\.apple\.security\.application-groups<\/key>\s*\n\s*<array>\s*\n\s*<string>)[^<]*(<\/string>)/,
    `$1${brand.identifiers.appGroup}$2`,
  );
  if (next !== body) {
    writeFileSync(path, next);
    log.ok(`entitlements.mas.plist`);
  } else {
    log.skip(`entitlements.mas.plist — already current`);
  }
}

function applyEntitlements(brand) {
  // Per-target .entitlements: App Group (shared container), keychain
  // access group (shared credentials), and BGTask identifiers all derive
  // from brand.json. Anchored on the entitlement KEY / id suffix so the
  // rewrite never needs to know the previous brand.
  const files = [
    join(UI, 'ios', 'App', 'App', 'App.entitlements'),
    join(UI, 'ios', 'App', 'Extensions', 'AppShareExtension', 'AppShareExtension.entitlements'),
    join(UI, 'ios', 'App', 'Extensions', 'AppLiveActivity', 'AppLiveActivity.entitlements'),
    join(UI, 'ios', 'App', 'Extensions', 'AppWidget', 'AppWidget.entitlements'),
    join(UI, 'ios', 'App', 'WatchApp', 'WatchApp.entitlements'),
  ];
  let any = false;
  let present = 0;
  for (const path of files) {
    ...
    body = body.replace(
      /(<key>keychain-access-groups<\/key>\s*\n\s*<array>\s*\n\s*<string>\$\(AppIdentifierPrefix\))[^<]*(<\/string>)/,
      `$1${brand.identifiers.keychainService}$2`,
    );
    // BGTask ids: <bundleId>.<kind> -- matched by suffix, base-agnostic.
    body = body.replace(
      /<string>[A-Za-z0-9_.]*\.(refresh|maintenance)<\/string>/g,
      `<string>${brand.identifiers.bundleId}.$1</string>`,
    );
    if (writeIfChanged(path, body)) any = true;
  }
  any ? log.ok(`entitlements × ${present}`) : log.skip(`entitlements -- already current`);
}

function applyXcodeProject(brand) {
  // Every target's PRODUCT_BUNDLE_IDENTIFIER = <bundleId><suffix>. Recompute
  // each from brand.identifiers.bundleId + the recognised target suffix, so
  // a rebrand re-derives all of them (app + 4 extensions + their test
  // bundles) with no knowledge of the old base.
  const path = join(UI, 'ios', 'App', 'App.xcodeproj', 'project.pbxproj');
  if (!existsSync(path)) {
    log.skip(`project.pbxproj — missing`);
    return;
  }
  const SUFFIXES = [
    '.widgettests',
  ...
  if (next !== body) {
    ...
    log.skip(`project.pbxproj — already current`);
  }
}

function applyAndroidFastlane(brand) {
  // Android fastlane: PACKAGE_NAME (Fastfile) + the Appfile fallback both
  // mirror the Play Store applicationId == brand.identifiers.bundleId.
  const fastfile = join(UI, 'android', 'fastlane', 'Fastfile');
  const appfile = join(UI, 'android', 'fastlane', 'Appfile');
  let any = false;
  if (existsSync(fastfile)) {
    const body = readFileSync(fastfile, 'utf8');
    const next = body.replace(
      /PACKAGE_NAME = "[^"]*"/,
      `PACKAGE_NAME = "${brand.identifiers.bundleId}"`,
    );
    if (next !== body) {
      writeFileSync(fastfile, next);
      any = true;
    }
  }
  if (existsSync(appfile)) {
    const body = readFileSync(appfile, 'utf8');
    const next = body.replace(
      /(package_name\(ENV\["PLAY_STORE_PACKAGE_NAME"\] \|\| ")[^"]*("\))/,
      `$1${brand.identifiers.bundleId}$2`,
    );
    if (next !== body) {
      writeFileSync(appfile, next);
      any = true;
    }
  }
...
function apply() {
  ...
  applyWatchInfoPlist(brand);
  applyEntitlements(brand);
  applyXcodeProject(brand);
