...
const SKIP_ROLE_PATTERNS = [
  ...
  {
    re: /^(director|vp|vice president|head of|chief)\b/i,
    reason: 'archetype mismatch: leadership role',
  },
  { re: /\bmanager,/i, reason: 'archetype mismatch: management role' },
  // Sales-adjacent / customer-facing
  {
    re: /\bsolutions? architect\b/i,
    reason: 'archetype mismatch: solutions architect (sales-adjacent)',
  },
  {
    re: /\bsolutions? engineer\b/i,
    reason: 'archetype mismatch: solutions engineer (sales-adjacent)',
  },
  ...
  {
    re: /\bdeveloper (advocate|relations)\b|\bdevrel\b/i,
    reason: 'archetype mismatch: DevRel/advocacy',
  },
  { re: /\btechnical writer\b/i, reason: 'archetype mismatch: technical writer' },
  {
    re: /\bcustomer (success|engineer|engineering)\b/i,
    reason: 'archetype mismatch: customer success',
  },
  { re: /\bfield engineer\b/i, reason: 'archetype mismatch: field engineering' },
  {
    re: /\bforward deployed\b/i,
    reason: 'archetype mismatch: forward deployed (client-facing, not core IC)',
  },
  // Research / ML-deep
  {
    re: /\bresearch (engineer|scientist|software engineer|platform engineer|lead)\b/i,
    reason: 'archetype mismatch: research role',
  },
  { re: /\bml (researcher|scientist)\b/i, reason: 'archetype mismatch: ML research' },
  {
    re: /\bml infrastructure\b|\bml ops\b|\bmachine learning platform\b|\bml platform\b|\bmachine learning infrastructure\b/i,
    reason: 'archetype mismatch: ML infra/platform (not Cole TS profile)',
  },
  {
    re: /\bml engineer\b|\bmachine learning engineer\b|\bmachine learning software engineer\b|\bsoftware engineer,\s*machine learning\b/i,
    reason: 'archetype mismatch: ML engineering',
  },
  {
    re: /\binference (engineer|deployment|services|infrastructure)\b/i,
    reason: 'archetype mismatch: inference/ML systems',
  },
  {
    re: /\bgpu (infrastructure|engineer|kernel)\b|\bhpc\b/i,
    reason: 'archetype mismatch: GPU/HPC infra',
  },
  {
    re: /\b(pre|post)[\s-]?training\b|\bpretraining\b|\bposttraining\b/i,
    reason: 'archetype mismatch: ML pre/post-training',
  },
  {
    re: /\bmodel (behavior|behaviour|efficiency|evaluation|architect)\b/i,
    reason: 'archetype mismatch: ML modeling/evaluation',
  },
  ...
  {
    re: /\b(synthetic data|multimodal ai|sovereign ai|vlm|alignment researcher|adversarial researcher|interpretability)\b/i,
    reason: 'archetype mismatch: AI lab ML research',
  },
  ...
  // AI-lab "Member of Technical Staff" research variants (keep MTS engineering roles)
  {
    ...
    reason: 'archetype mismatch: AI-lab MTS ML/research role',
  },
  // Voice synthesis ML (kept voice-agent product roles at TS companies)
  {
    re: /\bvoices\b|\bfuture\s*voices\b|\bvoice synthesis\b|\bspeech synthesis\b|\btext[\s-]?to[\s-]?speech\b/i,
    reason: 'archetype mismatch: voice/speech synthesis ML',
  },
  // Language specialists (Cole is TS-first)
  { re: /\bc\+\+/i, reason: 'archetype mismatch: C++ specialist (Cole TS-first)' },
  {
    re: /\(rust\)|\brust engineer\b|\(rust\s/i,
    reason: 'archetype mismatch: Rust specialist (Cole TS-first)',
  },
  {
    re: /\(golang\)|\(go\)|\bgolang engineer\b|\(golang\s/i,
    reason: 'archetype mismatch: Go/Golang specialist (Cole TS-first)',
  },
  // Robotics / vehicle / kernel low-level
  {
    re: /\brobot software\b|\bos & kernel\b|\bsensor systems\b|\bvehicle sw\b|\bvehicle software\b|\bwayve labs\b/i,
    reason: 'hard_no: robotics/embedded/vehicle/research-labs low-level',
  },
  // Non-IC specialists / admin
  {
    re: /\bknowledge\s*&\s*enablement\b|\benablement specialist\b/i,
    reason: 'archetype mismatch: enablement specialist',
  },
  ...
  {
    re: /\bproduct manager\b|\bprogram manager\b|\bproject manager\b/i,
    reason: 'archetype mismatch: PM role',
  },
  {
    re: /\brecruiter\b|\bpeople (operations|partner)\b|\btalent\b/i,
    reason: 'archetype mismatch: people/ops role',
  },
  // Hard_no per profile.yml
  { re: /\bmobile\b|\bios\b|\bandroid\b/i, reason: 'hard_no: native mobile (iOS/Android)' },
  {
    re: /\bhardware\b|\bembedded\b|\bfpga\b|\brobotics\b|\bfirmware\b/i,
    reason: 'hard_no: hardware/embedded',
  },
  { re: /\b(web3|crypto|blockchain)\b/i, reason: 'hard_no: crypto/web3' },
  {
    re: /\b(junior|early career|new grad|graduate|intern|internship|entry[\s-]level)\b/i,
    reason: 'hard_no: junior/mid title',
  },
  // HARD STOP roles per profile (clearance-bound)
  {
    re: /\bnational security\b|\bdefense\b|\bpublic sector\b|\bfederal\b|\bgovtech\b|\bgovernment\b/i,
    reason: 'HARD STOP: clearance-bound role per BG policy',
  },
  {
    re: /\bclearance\b|\btop secret\b|\bts\/sci\b|\bpolygraph\b/i,
    reason: 'HARD STOP: security clearance required',
  },
  // Specific niche skips
  { re: /\bqa engineer\b|\bqa engineering\b/i, reason: 'archetype mismatch: QA' },
  {
    re: /\bdata engineer\b|\bdata scientist\b|\banalytics engineer\b/i,
    reason: 'archetype mismatch: data engineering',
  },
  { re: /\bdesigner\b/i, reason: 'archetype mismatch: design role' },
  {
    re: /\bsecurity researcher\b|\bsecurity analyst\b/i,
    reason: 'archetype mismatch: security research',
  },
  {
    re: /\bdetection (and )?response\b|\bdetection & response\b/i,
    reason: 'archetype mismatch: security ops/SOC',
  },
  {
    re: /\baccount abuse\b|\btrust (and|&) safety\b|\btrust & safety\b/i,
    reason: 'archetype mismatch: T&S/abuse (heavy BG)',
  },
  { re: /\bgrc\b/i, reason: 'archetype mismatch: GRC' },
  // Healthcare PHI exposure → HARD STOP per profile
  {
    re: /\bhealthcare\b.*\bagent\b|\bclinical\b/i,
    reason: 'HARD STOP: healthcare/clinical (PHI exposure)',
  },
];
...
function classify(company, role) {
  ...
  if (HARD_STOP_COMPANIES.has(company)) {
    return {
      decision: 'skip',
      tier: 'HARD STOP',
      reason: `HARD STOP: ${company} is defense/intel — clearance-required`,
    };
  }
  ...
  if (HIGH_BG_COMPANIES.has(company)) {
    return {
      decision: 'skip',
      tier: 'HIGH BG',
      reason: `HIGH BG risk: ${company} (AI lab / fintech / large public US tech per BG policy)`,
    };
  }
  ...
  for (const { re, reason } of SKIP_ROLE_PATTERNS) {
    if (re.test(role)) {
      return {
        decision: 'skip',
        tier: reason.startsWith('hard_no:') ? 'hard_no' : 'archetype',
        reason,
      };
    }
  }
...
for (const [c, n] of Object.entries(survByCompany)
  .sort((a, b) => b[1] - a[1])
  .slice(0, 30)) {
  console.log(`  ${n}\t${c}`);
}
