...
Reference files that demonstrate Heron data formats and conventions. None of these are used at runtime -- they exist so you can see the expected structure before creating your own files.
...
These files are read-only references. To set up your own Heron instance:

1. Run `pnpm doctor` to check prerequisites.
...
4. Copy `templates/profile.example.yml` → `config/profile.yml` and edit.
