...
1. **File → New → Target → watchOS → App**
   ...
   - Bundle ID: `<brand.json::identifiers.bundleId>.watchkitapp` -- currently `com.resistjs.heron.watchkitapp`
...
3. Under the watch target's **Signing & Capabilities**:
   ...
   - Add **App Groups**: `<brand.json::identifiers.appGroup>` -- currently `group.com.resistjs.heron`
