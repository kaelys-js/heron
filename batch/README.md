...
4. **Results** are automatically merged into `data/applications.md` and verified with `the pipeline.integration test` at the end of the run.
...
4. After all workers finish, batch-runner calls `merge-tracker.mjs` to merge TSVs into `data/applications.md` and runs `the pipeline.integration test` to check integrity.
